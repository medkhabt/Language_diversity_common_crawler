...
static void Schedule(GtkObject * object, gpointer func_data)
{
  ...
  cfg.time->tm_hour = gtk_spin_button_get_value_as_int(GTK_SPIN_BUTTON(gui_cfg.hour_label));
  cfg.time->tm_min = gtk_spin_button_get_value_as_int(GTK_SPIN_BUTTON(gui_cfg.min_label));

  guint year, mon, day;
  gtk_calendar_get_date(GTK_CALENDAR(gui_cfg.calendar), &year, &mon, &day);
  cfg.time->tm_year = year - 1900;
  cfg.time->tm_mon = mon;
  cfg.time->tm_mday = day;
  ...
  cfg.reschedh = gtk_spin_button_get_value_as_int(GTK_SPIN_BUTTON(gui_cfg.resched));
...
void gui_build_scheduler(int popup)
{
  ...
  gtk_window_set_title(GTK_WINDOW(gui_cfg.cfg_sch), gettext("Pavuk: Scheduler"));
  gtk_widget_realize(gui_cfg.cfg_sch);
  gtk_signal_connect(GTK_OBJECT(gui_cfg.cfg_sch), "destroy", GTK_SIGNAL_FUNC(gtk_widget_destroyed), &gui_cfg.cfg_sch);

  col = guitl_timesel_new(&gui_cfg.calendar, &gui_cfg.hour_label, &gui_cfg.min_label, &gui_cfg.month_combo, &gui_cfg.year_label);
  ...
  gui_cfg.sched_cmd = guitl_tab_add_entry(ptab, gettext("Scheduling command: "), 0, 0, FALSE);
  ...
  gui_cfg.resched = gtk_spin_button_new((GtkAdjustment *) gtk_adjustment_new(0.0, 0.0, 10000.0, 1.0, 5.0, 0.0), 10.0, 0.0);
  ...
  gtk_signal_connect(GTK_OBJECT(button), "clicked", GTK_SIGNAL_FUNC(Schedule), (gpointer) NULL);
  ...
  gtk_container_add(GTK_CONTAINER(brow), button);
  gtk_signal_connect(GTK_OBJECT(button), "clicked", GTK_SIGNAL_FUNC(guitl_PopdownW), (gpointer) gui_cfg.cfg_sch);
  GTK_WIDGET_SET_FLAGS(button, GTK_CAN_DEFAULT);
  gtk_widget_show(button);
