...
static void gtk_multicol_get_arg(GtkObject * object, GtkArg * arg, guint arg_id);
static void gtk_multicol_set_arg(GtkObject * object, GtkArg * arg, guint arg_id);
...
static void gtk_multicol_class_init(GtkMultiColClass * class)
{
  ...
  gtk_object_add_arg_type("GtkBox::hspacing", GTK_TYPE_INT, GTK_ARG_READWRITE, ARG_HSPACING);
  gtk_object_add_arg_type("GtkBox::vspacing", GTK_TYPE_INT, GTK_ARG_READWRITE, ARG_VSPACING);
...
static void gtk_multicol_set_arg(GtkObject * object, GtkArg * arg, guint arg_id)
{
  ...
  {
  case ARG_VSPACING:
    gtk_multicol_set_spacing(multicol, multicol->hspacing, GTK_VALUE_INT(*arg));
    break;
  case ARG_HSPACING:
    gtk_multicol_set_spacing(multicol, GTK_VALUE_INT(*arg), multicol->vspacing);
    break;
  default:
    arg->type = GTK_TYPE_INVALID;
    break;
...
static void gtk_multicol_size_request(GtkWidget * widget, GtkRequisition * requisition)
{
  ...
  while(children)
  {
    ...
    if(GTK_WIDGET_VISIBLE(child->widget))
    {
      ...
      maxwidth = TL_MAX(maxwidth, (child->widget->requisition.width + child->padding * 2));
      maxheight = TL_MAX(maxheight, child->widget->requisition.height + child->padding * 2);
  ...
  if(nm)
  {
    requisition->width = ncol * (maxwidth + multicol->hspacing) + multicol->hspacing;

    requisition->height = (nm / ncol + (nm % ncol != 0)) * (maxheight + multicol->vspacing) + multicol->vspacing;
  }
...
static void gtk_multicol_size_allocate(GtkWidget * widget, GtkAllocation * allocation)
{
  ...
  if(nm)
  {
    if(multicol->nrows)
      ...
      nrow = (allocation->height - multicol->hspacing) / (maxheight + multicol->hspacing);
    ...
    while(children)
    {
      ...
      {
        child_allocation.x = allocation->x + multicol->hspacing + col * (maxwidth + multicol->hspacing);
        child_allocation.y = allocation->y + multicol->vspacing + row * (maxheight + multicol->vspacing);
