...
static void limtab_tree(GtkWidget * notebook)
{
  ...
  gui_cfg.cgi_sw = gtk_check_button_new_with_label(gettext("Download cgi-generated pages"));
  gtk_widget_show(gui_cfg.cgi_sw);
  gtk_table_attach(GTK_TABLE(ptab), gui_cfg.cgi_sw, 0, 1, 0, 1, GTK_FILL, GTK_FILL, 5, 0);

  gui_cfg.ftpd_sw = gtk_check_button_new_with_label(gettext("Recurse through FTP directory"));
  gtk_widget_show(gui_cfg.ftpd_sw);
  gtk_table_attach(GTK_TABLE(ptab), gui_cfg.ftpd_sw, 0, 1, 1, 2, GTK_FILL, GTK_FILL, 5, 0);

  gui_cfg.robots_sw = gtk_check_button_new_with_label(gettext("Allow \"robots.txt\""));
  gtk_widget_show(gui_cfg.robots_sw);
  gtk_table_attach(GTK_TABLE(ptab), gui_cfg.robots_sw, 0, 1, 2, 3, GTK_FILL, GTK_FILL, 5, 0);

  gui_cfg.ftp_html = gtk_check_button_new_with_label(gettext("Process HTML files downloaded over FTP"));
  gtk_widget_show(gui_cfg.ftp_html);
  gtk_table_attach(GTK_TABLE(ptab), gui_cfg.ftp_html, 0, 1, 3, 4, GTK_FILL, GTK_FILL, 5, 0);

  gui_cfg.leaves_sw = gtk_check_button_new_with_label(gettext("Don't leave starting site"));
  gtk_widget_show(gui_cfg.leaves_sw);
  gtk_table_attach(GTK_TABLE(ptab), gui_cfg.leaves_sw, 1, 2, 0, 1, GTK_FILL, GTK_FILL, 5, 0);

  gui_cfg.leaved_sw = gtk_check_button_new_with_label(gettext("Don't leave starting directory"));
  gtk_widget_show(gui_cfg.leaved_sw);
  gtk_table_attach(GTK_TABLE(ptab), gui_cfg.leaved_sw, 1, 2, 1, 2, GTK_FILL, GTK_FILL, 5, 0);

  gui_cfg.dont_leave_site_dir = gtk_check_button_new_with_label(gettext("Don't leave site enter directory"));
  gtk_widget_show(gui_cfg.dont_leave_site_dir);
  gtk_table_attach(GTK_TABLE(ptab), gui_cfg.dont_leave_site_dir, 1, 2, 2, 3, GTK_FILL, GTK_FILL, 5, 0);

  gui_cfg.singlepage = gtk_check_button_new_with_label(gettext("Download just single page"));
  gtk_widget_show(gui_cfg.singlepage);
  gtk_table_attach(GTK_TABLE(ptab), gui_cfg.singlepage, 1, 2, 3, 4, GTK_FILL, GTK_FILL, 5, 0);

  gui_cfg.limit_inlines = gtk_check_button_new_with_label(gettext("Apply limiting options on inline objects"));
  gtk_widget_show(gui_cfg.limit_inlines);
  gtk_table_attach(GTK_TABLE(ptab), gui_cfg.limit_inlines, 0, 1, 4, 5, GTK_FILL, GTK_FILL, 5, 0);
  ...
  gui_cfg.maxdoc_label = guitl_tab_add_numentry(ptab, gettext("Max. count of documents: "), 0, 4, INT_MAX);

  gui_cfg.maxlev_label = guitl_tab_add_numentry(ptab, gettext("Max. depth of tree: "), 0, 5, USHRT_MAX);

  gui_cfg.leave_level = guitl_tab_add_numentry(ptab, gettext("Max. levels to leave from starting site: "), 0, 6, USHRT_MAX);

  gui_cfg.maxsize_label = guitl_tab_add_numentry(ptab, gettext("Max. document size: "), 2, 4, INT_MAX);

  gui_cfg.min_size = guitl_tab_add_numentry(ptab, gettext("Min. document size: "), 2, 5, INT_MAX);

  gui_cfg.site_level = guitl_tab_add_numentry(ptab, gettext("Max. site levels to leave from starting site: "), 2, 6, INT_MAX);
  ...
  gui_cfg.subdir_label = guitl_tab_add_path_entry(ptab, gettext("Working subdirectory :"), 0, 0, TRUE);

  gui_cfg.en_uexit = guitl_tab_add_entry(ptab, gettext("User condition script: "), 0, 1, FALSE);

  gui_cfg.follow_cmd = guitl_tab_add_entry(ptab, gettext("Follow command: "), 0, 2, FALSE);
...
static void limtab_patterns(GtkWidget * notebook)
{
  ...
  gui_cfg.pattern_label = guitl_tab_add_edit_entry(col, gettext("Documents matching pattern: "), NULL, 0, 0, FALSE);

  gui_cfg.skip_pattern = guitl_tab_add_edit_entry(col,
    gettext("skip: "), gettext("Pavuk: edit Documents matching skip pattern"), 2, 0, FALSE);

  gui_cfg.url_pattern_label = guitl_tab_add_edit_entry(col, gettext("URL matching pattern: "), NULL, 0, 1, FALSE);

  gui_cfg.skip_url_pattern = guitl_tab_add_edit_entry(col,
    gettext("skip: "), gettext("Pavuk: edit URL matching skip pattern"), 2, 1, FALSE);
  ...
  gui_cfg.rpattern = guitl_tab_add_edit_entry(col, gettext("Documents matching pattern: "), NULL, 0, 0, TRUE);

  gui_cfg.skip_rpattern = guitl_tab_add_edit_entry(col,
    gettext("skip: "), gettext("Pavuk: edit Documents matching skip pattern"), 2, 0, TRUE);

  gui_cfg.url_rpattern = guitl_tab_add_edit_entry(col, gettext("URL matching pattern: "), NULL, 0, 1, TRUE);

  gui_cfg.url_skip_rpattern = guitl_tab_add_edit_entry(col,
    gettext("skip: "), gettext("Pavuk: edit URL matching skip pattern"), 2, 1, TRUE);
#endif
}
...
static void limtab_hosts(GtkWidget * notebook)
{
  ...
  gtk_widget_show(col);
  gtk_table_attach(GTK_TABLE(box), col, 0, 1, 0, 1, GTK_FILL | GTK_EXPAND, GTK_FILL | GTK_EXPAND, 5, 5);

  gui_cfg.hosts_sw = gtk_check_button_new_with_label(gettext("Allow / Disallow sites"));
  gtk_widget_show(gui_cfg.hosts_sw);
  gtk_table_attach(GTK_TABLE(col), gui_cfg.hosts_sw, 0, 1, 0, 1, GTK_FILL, GTK_FILL, 5, 5);

  brow = guitl_new_edit_list(&gui_cfg.hosts_list, &gui_cfg.hosts_entry, gettext("Site: "), NULL, NULL, NULL, NULL, TRUE, NULL);

  gtk_table_attach(GTK_TABLE(col), brow, 0, 1, 1, 2, GTK_FILL | GTK_EXPAND, GTK_FILL | GTK_EXPAND, 5, 5);

  col = gtk_table_new(2, 1, FALSE);
  gtk_widget_show(col);
  gtk_table_attach(GTK_TABLE(box), col, 1, 2, 0, 1, GTK_FILL | GTK_EXPAND, GTK_FILL | GTK_EXPAND, 5, 5);

  gui_cfg.domain_sw = gtk_check_button_new_with_label(gettext("Allow / Disallow domains"));
  gtk_widget_show(gui_cfg.domain_sw);
  gtk_table_attach(GTK_TABLE(col), gui_cfg.domain_sw, 0, 1, 0, 1, GTK_FILL, GTK_FILL, 5, 5);

  brow = guitl_new_edit_list(&gui_cfg.domain_list, &gui_cfg.domain_entry, gettext("Domain: "), NULL, NULL, NULL, NULL, TRUE, NULL);

  gtk_table_attach(GTK_TABLE(col), brow, 0, 1, 1, 2, GTK_FILL | GTK_EXPAND, GTK_FILL | GTK_EXPAND, 5, 5);

#ifdef HAVE_REGEX
  ptab = gtk_frame_new(gettext("IP address RE patterns"));
  gtk_table_attach(GTK_TABLE(box), ptab, 0, 2, 1, 2, GTK_FILL, GTK_FILL, 5, 5);
  gtk_widget_show(ptab);
  ...
  gui_cfg.aip = guitl_tab_add_edit_entry(col, gettext("Server IP address matching pattern: "), NULL, 0, 0, TRUE);

  gui_cfg.skipip = guitl_tab_add_edit_entry(col,
    gettext("skip: "), gettext("Pavuk: edit Server IP address matching skip pattern"), 2, 0, TRUE);
#endif

  ptab = gtk_frame_new(gettext("Server ports"));
  gtk_table_attach(GTK_TABLE(box), ptab, 0, 2, 2, 3, GTK_FILL, GTK_FILL, 5, 5);
  gtk_widget_show(ptab);
  ...
  gui_cfg.allow_ports = gtk_check_button_new_with_label(gettext("Allow/deny"));
  gtk_table_attach(GTK_TABLE(col), gui_cfg.allow_ports, 0, 1, 0, 1, GTK_FILL, GTK_FILL, 1, 1);
  gtk_widget_show(gui_cfg.allow_ports);

  gui_cfg.ports = guitl_tab_add_edit_entry(col, gettext("Ports: "), NULL, 1, 0, FALSE);
}
...
static void limtab_docs(GtkWidget * notebook)
{
  ...
  gui_cfg.sufix_sw = gtk_check_button_new_with_label(gettext("Allow / Disallow suffix"));
  gtk_widget_show(gui_cfg.sufix_sw);
  gtk_table_attach(GTK_TABLE(col), gui_cfg.sufix_sw, 0, 1, 0, 1, GTK_FILL, GTK_FILL, 5, 5);

  brow = guitl_new_edit_list(&gui_cfg.sufixlist, &gui_cfg.sufix_label, gettext("Suffix: "), NULL, NULL, NULL, NULL, TRUE, NULL);
  ...
  gui_cfg.prefix_sw = gtk_check_button_new_with_label(gettext("Allow / Disallow prefix"));
  gtk_widget_show(gui_cfg.prefix_sw);
  gtk_table_attach(GTK_TABLE(col), gui_cfg.prefix_sw, 0, 1, 0, 1, GTK_FILL, GTK_FILL, 5, 5);

  brow = guitl_new_edit_list(&gui_cfg.prefixlist, &gui_cfg.prefix_label, gettext("Prefix: "), NULL, NULL, NULL, NULL, TRUE, NULL);
...
static void limtab_mime(GtkWidget * notebook)
{
  ...
  gui_cfg.mime_sw = gtk_check_button_new_with_label(gettext("Allow / Disallow MIME type"));
  ...
  col = guitl_new_edit_list(&gui_cfg.amimelist, &entry,
    gettext("MIME type: "), NULL, NULL, NULL, NULL, TRUE, get_mimetypes_collection());
...
static void limtab_time(GtkWidget * notebook)
{
  ...
  gui_cfg.btime_sw = gtk_check_button_new_with_label(gettext("Check if doc. time newer than this"));
  ...
  gui_cfg.etime_sw = gtk_check_button_new_with_label(gettext("Check if doc. time older than this"));
  ...
  gui_cfg.max_time = guitl_tab_add_doubleentry(frame, gettext("Maximal allowed time of downloading: "), 0, 0, INT_MAX, 2);

  label = gtk_label_new(gettext(" min"));
  gtk_table_attach(GTK_TABLE(frame), label, 2, 3, 0, 1, GTK_SHRINK, GTK_FILL, 2, 2);
...
static void limtab_html(GtkWidget * notebook)
{
  ...
  for(i = 0; i < html_link_tags_num(); i++)
  {
    for(j = 0; html_link_tags[i].attribs[j].attrib; j++)
    {
      ...
      {
        snprintf(pom, sizeof(pom), gettext("%s of %s"), html_link_tags[i].attribs[j].attrib, html_link_tags[i].tag);
        pom[sizeof(pom) - 1] = 0;

        check = gtk_check_button_new_with_label(pom);
        gtk_object_set_user_data(GTK_OBJECT(check), (gpointer) & html_link_tags[i].attribs[j].stat);
...
static void TagPatNew(int row)
{
  ...
  type = GTK_OPTION_MENU(tagpat.re_wc)->menu_item ?
    (int) gtk_object_get_user_data(GTK_OBJECT(GTK_OPTION_MENU(tagpat.re_wc)->menu_item)) : TAGP_WC;
...
static void limtab_tags(GtkWidget * notebook)
{
  ...
  gtk_clist_set_column_title(GTK_CLIST(tagpat.list), 1, gettext("Tag pattern"));
  gtk_clist_set_column_title(GTK_CLIST(tagpat.list), 2, gettext("Attribute pattern"));
  gtk_clist_set_column_title(GTK_CLIST(tagpat.list), 3, gettext("URL pattern"));
  ...
  gtk_signal_connect(GTK_OBJECT(tagpat.list), "select_row", GTK_SIGNAL_FUNC(TagPatSelectRow), NULL);
  ...
  gtk_table_attach(GTK_TABLE(ptab), label, 0, 1, 0, 1, GTK_FILL, GTK_FILL, 2, 2);
  gtk_widget_show(label);

  tagpat.re_wc = gtk_option_menu_new();
  gtk_table_attach(GTK_TABLE(ptab), tagpat.re_wc, 1, 2, 0, 1, GTK_FILL, GTK_FILL, 2, 2);
...
*/
  pp2 = NULL;
  k = 0;
  pp1 = _malloc((html_link_tags_num() + 2) * sizeof(pp1[0]));
  for(i = 0; i < html_link_tags_num(); i++)
  {
  ...
  tagpat.tag = guitl_tab_add_enum(ptab, gettext("Tag pattern"), 0, 1, (const char **) pp1, TRUE);
  _free(pp1);

  tagpat.attrib = guitl_tab_add_enum(ptab, gettext("Attribute pattern"), 0, 2, (const char **) pp2, TRUE);
  _free(pp2);

  tagpat.urlp = guitl_tab_add_entry(ptab, gettext("URL pattern"), 0, 3, FALSE);
  ...
  gtk_signal_connect(GTK_OBJECT(button), "clicked", GTK_SIGNAL_FUNC(TagPatAppend), &tagpat);

  button = guitl_pixmap_button(modify_xpm, NULL, gettext("Modify"));
  gtk_container_add(GTK_CONTAINER(bbox), button);
  gtk_widget_show(button);
  gtk_signal_connect(GTK_OBJECT(button), "clicked", GTK_SIGNAL_FUNC(TagPatModify), &tagpat);

  button = guitl_pixmap_button(clear_xpm, NULL, gettext("Clear"));
  gtk_container_add(GTK_CONTAINER(bbox), button);
  gtk_widget_show(button);
  gtk_signal_connect(GTK_OBJECT(button), "clicked", GTK_SIGNAL_FUNC(guitl_ListClear), tagpat.list);

  button = guitl_pixmap_button(delete_xpm, NULL, gettext("Delete"));
  gtk_container_add(GTK_CONTAINER(bbox), button);
  gtk_widget_show(button);
  gtk_signal_connect(GTK_OBJECT(button), "clicked", GTK_SIGNAL_FUNC(guitl_ListDeleteSelected), tagpat.list);
...
void gui_build_config_limits(int popup)
{
  ...
  gtk_window_set_title(GTK_WINDOW(gui_cfg.cfg_limits), gettext("Pavuk: Limits config"));
  gtk_widget_realize(gui_cfg.cfg_limits);
  gtk_signal_connect(GTK_OBJECT(gui_cfg.cfg_limits), "destroy", GTK_SIGNAL_FUNC(gtk_widget_destroyed), &gui_cfg.cfg_limits);
  ...
  gtk_signal_connect(GTK_OBJECT(button), "clicked", GTK_SIGNAL_FUNC(gui_PopdownWC), (gpointer) gui_cfg.cfg_limits);
  ...
  gtk_container_add(GTK_CONTAINER(brow), button);
  gtk_signal_connect(GTK_OBJECT(button), "clicked", GTK_SIGNAL_FUNC(CfgLimits), (gpointer) gui_cfg.config_shell);
  GTK_WIDGET_SET_FLAGS(button, GTK_CAN_DEFAULT);
  gtk_widget_show(button);

  button = guitl_pixmap_button(common_xpm, NULL, gettext("Common ..."));
  gtk_container_add(GTK_CONTAINER(brow), button);
  gtk_signal_connect(GTK_OBJECT(button), "clicked", GTK_SIGNAL_FUNC(gui_PopupW), (gpointer) PAVUK_CFGCOMM);
  GTK_WIDGET_SET_FLAGS(button, GTK_CAN_DEFAULT);
  gtk_widget_show(button);
  ...
  gtk_container_add(GTK_CONTAINER(brow), button);
  gtk_signal_connect(GTK_OBJECT(button), "clicked", GTK_SIGNAL_FUNC(guitl_PopdownW), (gpointer) gui_cfg.cfg_limits);
  GTK_WIDGET_SET_FLAGS(button, GTK_CAN_DEFAULT);
  gtk_widget_show(button);
