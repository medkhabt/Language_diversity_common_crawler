...
dlhash *dlhash_new(unsigned int size, dlkey_func key_func, dlhash_func hash_func, dlcomp_func comp_func)
{
  dlhash *retv = _malloc(sizeof(*retv));
  ASSERT(retv != NULL);
  ...
  retv->nodes = (dllist **) _malloc(size * sizeof(retv->nodes[0]));
  ASSERT(retv->nodes != NULL);
  memset(retv->nodes, 0, size * sizeof(retv->nodes[0]));

  return retv;
}

void dlhash_set_free_func(dlhash * hash, dlfree_func free_func, dlkeyfree_func keyfree_func)
{
  ASSERT(hash);
...
void dlhash_resize(dlhash * hash, unsigned int new_size)
{
  ...
  hash->size = new_size;
  hash->nodes = (dllist **) _malloc(new_size * sizeof(hash->nodes[0]));
  ASSERT(hash->nodes != NULL);
  memset(hash->nodes, 0, new_size * sizeof(hash->nodes[0]));
