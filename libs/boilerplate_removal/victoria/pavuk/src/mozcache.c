...
static void moz_old_cache_read_db(char *dirname)
{
  ...
  if(dirname)
  {
    ...
    snprintf(pom, sizeof(pom), "%s%scache.db", dirname, (tl_is_dirname(dirname) ? "" : "/"));
    pom[sizeof(pom) - 1] = 0;
    ...
    if(!reopen && !tl_stat(pom, &estat) && estat.st_mtime != moz_cache_db_modtime)
    {
      reopen = TRUE;
    }
  }
  ...
  if(reopen)
  {
    ...
    moz_cache_db = dbopen(pom, O_BINARY | O_RDONLY, S_IRUSR | S_IWUSR, DB_HASH, &hash_info);
    ...
    else
    {
      ...
      while(!moz_cache_db->seq(moz_cache_db, &key, &data, R_NEXT))
      {
        ...
        mp = moz_pair_new(moz_old_cache_get_filename(&data), moz_old_cache_get_urlstr(&data));
...
static int moz2_open_db(char *name)
{
  ...
  for(i = 0; i < MOZ2_LAST; i++)
  {
    ...
    moz2_files[i].fd = tl_open(pom, O_BINARY | O_RDWR, S_IRUSR | S_IWUSR | S_IRGRP | S_IWGRP | S_IROTH | S_IWOTH);
    if(moz2_files[i].fd < 0)
      moz2_files[i].fd = tl_open(pom, O_BINARY | O_RDONLY, S_IRUSR | S_IWUSR | S_IRGRP | S_IWGRP | S_IROTH | S_IWOTH);

    if(moz2_files[i].fd < 0)
    {
      xvperror(DBGvars(NULL), gettext("Error opening cache index file %s\n"), mk_native(pom));
      rv = -1;
      break;
    }
  }
  ...
  if(!rv)
  {
    ...
    if((read(moz2_files[MOZ2_MAP_FILE].fd, bucket, sizeof(bucket)) != sizeof(bucket)) || (ntohl(cmh->version) != 0x00010003))
    {
...
static char *moz2_get_filename(int type, uint32__t hashnr, uint32__t generation)
{
  char pom[PATH_MAX];
...
static moz2_cache_record_header *moz2_load_record_data(moz2_cache_map_record * rec)
{
  char *p;
  ...
  if(!MOZ2_LOC_FILE(rec->meta_loc))
  {
    ...
    fn = moz2_get_filename('m', rec->hashnr, MOZ2_LOC_GENERATION(rec->meta_loc));
  ...
  {
    p = load_blocks(moz2_files[MOZ2_LOC_FILE(rec->meta_loc)].id, MOZ2_LOC_BLOCKNR(rec->meta_loc), MOZ2_LOC_BLOCKS(rec->meta_loc));
  }

  return (moz2_cache_record_header *) p;
}

static moz_pair_t *moz2_compose_record(moz2_cache_map_record * rec, moz2_cache_record_header * meta)
{
  moz_pair_t *ret = NULL;
  char *u, *p;
...
static void moz_new_cache_read_db(char *dirname)
{
  ...
  if(dirname)
  {
    ...
    snprintf(pom, sizeof(pom), "%s%s_CACHE_MAP_", dirname, (tl_is_dirname(dirname) ? "" : "/"));
    pom[sizeof(pom) - 1] = 0;
...
static void moz_cache_read_db(char *dirname)
{
  ...
  if(!moz_hash)
  {
    moz_hash = dlhash_new(256, moz_pair_get_urlstr, str_hash_func, str_comp_func);
