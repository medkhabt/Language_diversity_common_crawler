...
#if defined(HAVE_MOZJS) && defined(HAVE_JSAPI_H)
...
#define SET_STR(s) \
  if (s) \
    *val = STRING_TO_JSVAL(JS_NewStringCopyZ(cx, s)); \
  else \
    *val = JSVAL_NULL;
...
static void pjs_error(JSContext * cx, const char *msg, JSErrorReport * rpt)
{
  if(!rpt)
    ...
    xprintf(1, "pjs: %s: line %i: error: %s\n", mk_native(rpt->filename), rpt->lineno, msg);
}
...
static JSBool pjs_url_get_parent(JSContext *, JSObject *, uintN, jsval *, jsval *);
static JSBool pjs_url_check_cond(JSContext *, JSObject *, uintN, jsval *, jsval *);
...
static JSBool pjs_url_check_cond(JSContext * cx, JSObject * obj, uintN argc, jsval * argv, jsval * rv)
{
  ...
  if(r < 0)
  {
    JS_ReportError(cx, "pjs: PavukUrl.check_cond: %s - \"%s\"", gettext("unknown limiting condition"), name);
...
static JSObject *pjs_url_class_init(JSContext * cx, JSObject * obj)
{
  ...
  prot = JS_InitClass(cx, obj, NULL, &urlClass, pjs_url_new, 0, urlProperties, urlMethods, NULL, NULL);

  return prot;
}

/* PavukFnrules class implementation */
static JSBool pjs_fnrules_get_macro(JSContext *, JSObject *, uintN, jsval *, jsval *);
static JSBool pjs_fnrules_get_sub(JSContext *, JSObject *, uintN, jsval *, jsval *);
static JSBool pjs_fnrules_get_property(JSContext *, JSObject *, jsval, jsval *);
...
static JSPropertySpec fnrulesProperties[] = 
{
  ...
  {"pattern_type", PJSFNRULES_PATTERN_TYPE, JSPROP_ENUMERATE | JSPROP_READONLY},
  {NULL, 0, 0},
};
...
{
  JS_ReportError(cx, gettext("PavukFnrules constructor call prohibited from script\n"));
  return JS_TRUE;
}
...
static JSObject *pjs_fnrules_class_init(JSContext * cx, JSObject * obj)
{
  ...
  prot = JS_InitClass(cx, obj, NULL, &fnrulesClass, pjs_fnrules_new, 0, fnrulesProperties, fnrulesMethods, NULL, NULL);
...
int pjs_init(void)
{
  ...
  if(!rt)
  {
    ...
    pjs_have_pavuk_url_cond_check = pjs_func_exist(cx, go, "pavuk_url_cond_check");
  }
...
int pjs_execute(const char *scr)
{
  jsval rval;
  int rv;
