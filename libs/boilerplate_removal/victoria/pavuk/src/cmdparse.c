...
static char *tl_cmdln_parse_unescape_str(const char *str, int len)
{
  char *rv;
  char *p;
  int i;
...
static int tl_cmdln_parse_get_param(const char *str, char **param)
{
  int i, j;
  int was_start = FALSE;
  char delim = -1;
  ...
  *param = (delim == '\'') ? tl_strndup(p, j) : tl_cmdln_parse_unescape_str(p, j);

  return i + j + (delim ? 1 : 0);
}

char **tl_cmdln_parse(const char *cmdln)
{
  const char *p;
  char *param;
  char **rv = NULL;
  ...
  while(*p)
  {
    ...
    if(l < 0)
    {
      ...
      for(pp = rv; pp && *pp; pp++)
        _free(*pp);
      _free(rv);
      xprintf(0, gettext("Error parsing commandline at: %s\n"), p);
      return NULL;
    }
    if(!l)
      break;
...
int tl_win32_system(const char *cmd)
{
  if(!tl_access("/bin/sh", X_OK))
  {
    return system(cmd);
  }
  else
