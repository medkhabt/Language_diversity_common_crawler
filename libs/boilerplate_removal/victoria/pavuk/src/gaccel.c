...
{
  gaccel *retv = g_malloc(sizeof(*retv));
...
static void gaccel_widget_add_accel(GtkWidget * widget, guint sigid,
  GtkAccelGroup * agroup, guint akey, GdkModifierType amod, GtkAccelFlags aflag, gaccel * gaccelp)
{
  if((aflag & GTK_ACCEL_VISIBLE) && (sigid == gtk_signal_lookup(gaccelp->sigid, GTK_OBJECT_TYPE(widget))))
  {
...
static void gaccel_widget_remove_accel(GtkWidget * widget, GtkAccelGroup * agroup, guint akey, GdkModifierType amod, gaccel * gaccelp)
{
...
void gaccel_bind_widget(gchar * name, gchar * sigid, GtkWidget * widget, GtkAccelGroup * accel_group, GtkWidget * parent)
{
  ...
  {
    accel_group = (GtkAccelGroup *) gtk_object_get_data(GTK_OBJECT(parent), "gaccel_accel_group");

    if(!accel_group)
    {
      accel_group = gtk_accel_group_new();
      gtk_window_add_accel_group(GTK_WINDOW(gtk_widget_get_toplevel(parent)), accel_group);
      gtk_object_set_data_full(GTK_OBJECT(parent), "gaccel_accel_group", accel_group, (GtkDestroyNotify) gtk_accel_group_unref);
    }
  }
  ...
  gtk_signal_connect(GTK_OBJECT(widget), "destroy", GTK_SIGNAL_FUNC(gaccel_widget_destroy), gaccelp);
#if GTK_FACE < 2
  gtk_signal_connect(GTK_OBJECT(widget), "add_accelerator", GTK_SIGNAL_FUNC(gaccel_widget_add_accel), gaccelp);
  gtk_signal_connect(GTK_OBJECT(widget), "remove_accelerator", GTK_SIGNAL_FUNC(gaccel_widget_remove_accel), gaccelp);
#endif
}
