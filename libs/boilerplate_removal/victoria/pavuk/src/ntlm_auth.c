...
char *ntlm_get_t1msg_str(const char *domain, const char *host)
{
  int msgtk_size;
  int dlen, hlen;
  ...
  return base64_encode_data((void *) &msg, msgtk_size);
}
...
char *ntlm_get_nonce(char *t2msg, unsigned short *t2flags)
{
  ...
  msg = NULL;
  st = base64_decode_data(t2msg, (void **) &msg);

#ifndef NTLM_UNPACKED_STRUCT
  if(st < 0 || st < sizeof(ntlm_type_2_msg_t) || strcmp((char *) msg->protocol, "NTLMSSP") || msg->type != 0x02)
  {
    rv = NULL;
  }
  else
...
#else
  if(st < 0 || st < NTLM_MSG2_SIZE || strcmp(msg, "NTLMSSP") || msg[8] != 0x02)
  {
    rv = NULL;
  }
  else
  {
    ...
    memcpy(rv, msg + 24, 8);
    rv[8] = '\0';
    *t2flags = ntlm_do_litle_endian_short(*((unsigned short *) (((char *) msg) + 20)));
  }
#endif
  _free(msg);
...
#ifdef HAVE_MCRYPT
static void ntlm_des_enc(unsigned char *key, unsigned char *plaintext, unsigned char *result)
{
  MCRYPT ctx;
  unsigned char res[8];
 ...
 */
static void ntlm_calc_resp(unsigned char *keys, unsigned char *plaintext, unsigned char *results)
{
...
static void ntlm_gen_lm_resp(char *pass, char *nonce, char *buf)
{
  ...
  ntlm_calc_resp(lm_hpw, (unsigned char *) nonce, (unsigned char *) buf);
}
...
static void ntlm_gen_nt_resp(char *pass, char *nonce, char *buf)
{
  ...
  ntlm_calc_resp(nt_hpw, (unsigned char *) nonce, (unsigned char *) buf);

  _free(nt_pw);
}

static char *ntlm_get_t3msg_wcstr(char *domain, char *host, char *user, char *pass, char *nonce)
{
...
static char *ntlm_get_t3msg_asciistr(char *domain, char *host, char *user, char *pass, char *nonce)
{
  int dlen, hlen, ulen, lm_len, nt_len;
  int dofs, hofs, uofs, lm_ofs, nt_ofs;
  int msgtk_size;
...
char *ntlm_get_t3msg_str(char *domain, char *host, char *user, char *pass, char *nonce, unsigned short t2flags)
{
  return (t2flags & 0x01) 
	  ? ntlm_get_t3msg_wcstr(domain, host, user, pass, nonce) 
	  : ntlm_get_t3msg_asciistr(domain, host, user, pass, nonce);
}
...
int ntlm_negotiate_connection(doc * docp, char *authtag)
{
  ...
  {
    xprintf(1, gettext("NTLM authorization supported only on persistent connections!\n"));
  ...
  ai = authinfo_match_entry(docp->doc_url->type,
    url_get_site(docp->doc_url), url_get_port(docp->doc_url), url_get_path(docp->doc_url), NULL);
  domain = (ai && ai->ntlm_domain) ? ai->ntlm_domain : priv_cfg.auth_ntlm_domain;
  ...
  docp->additional_headers = tl_str_concat(DBGvars(NULL), "Authorization: NTLM ", ntlm_msg, "\r\n", NULL);
  _free(ntlm_msg);
  _free(docp->mime);
  _free(docp->type_str);

  /* [i_a] for a succesful NTLM negatiation, you'll need a persistent connection during the nego at least! */
  orig_was_persistent = docp->is_persistent;
  ...
  if(l)
  {
    docp->is_persistent &= orig_was_persistent; /* make sure non-persistent (erroneous) docs don't get reset to persistent again! */
    return -1;
  }
  ...
  {
    char *mstr = nw_strerror(docp, HTTP_BASE_ERRVAL + resp->ret_code);
	xprintf(1, gettext("Got unexpected response: %s:%s\n"), errcodetype(docp->errcode), mstr);
    docp->is_persistent &= orig_was_persistent; /* make sure non-persistent (erroneous) docs don't get reset to persistent again! */
    docp->errcode = ERR_HTTP_AUTH_NTLM;
  ...
  ASSERT(docp->is_persistent == TRUE);
  l = http_repeat_request(docp);
...
int ntlm_negotiate_proxy_connection(doc * docp, char *authtag)
{
  ...
  domain = (ai && ai->ntlm_domain) ? ai->ntlm_domain : priv_cfg.auth_proxy_ntlm_domain;
  ...
  docp->additional_headers = tl_str_concat(DBGvars(NULL), "Proxy-Authorization: NTLM ", ntlm_msg, "\r\n", NULL);
  ...
  {
    char *mstr = nw_strerror(docp, HTTP_BASE_ERRVAL + resp->ret_code);
	xprintf(1, gettext("Got unexpected response: %s:%s\n"), errcodetype(resp->ret_code), mstr);
    docp->is_persistent &= orig_was_persistent; /* make sure non-persistent (erroneous) docs don't get reset to persistent again! */
    docp->errcode = ERR_HTTP_PROAUTH_NTLM;
    http_response_free(resp);
    _free(mstr);
  ...
  if(!docp->is_persistent)
  {
    xprintf(1, gettext("NTLM proxy authorization supported only on persistent connections!\n"));
    docp->is_persistent &= orig_was_persistent; /* make sure non-persistent (erroneous) docs don't get reset to persistent again! */
    docp->errcode = ERR_HTTP_PROAUTH_NTLM;
    return -1;
  }
