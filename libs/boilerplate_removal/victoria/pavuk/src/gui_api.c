...
void gui_finish_document(doc * docp)
{
  ...
  if(cfg.xi_face && GTK_TOGGLE_BUTTON(gui_cfg.watch_download)->active)
  {
    GDK_THREADS_ENTER();
    LOCK_GTKTREE;
    gtk_ctree_select(GTK_CTREE(gui_cfg.tree_widget), docp->doc_url->tree_nfo[0]);
    gtk_ctree_node_moveto(GTK_CTREE(gui_cfg.tree_widget), docp->doc_url->tree_nfo[0], 0, 0.0, 0.0);
    UNLOCK_GTKTREE;
    GDK_THREADS_LEAVE();
  }
#endif
...
#if defined(__CYGWIN__) || defined(WIN32) || defined(__WIN32)
  snprintf(gtkrc, sizeof(gtkrc), "%s%s%s-gtkrc", cfg.install_path, (tl_is_dirname(cfg.install_path) ? "" : "/"), PACKAGE);
  gtkrc[sizeof(gtkrc) - 1] = 0;
  gtk_rc_add_default_file(gtkrc);
#endif
  /* Parse the file ~/.pavuk-gtkrc. This makes Pavuk themeable :) */
  if((p = getenv("HOME")))
  {
    snprintf(gtkrc, sizeof(gtkrc), "%s%s%s%s-gtkrc", p, 
		(tl_is_dirname(p) ? "" : "/"), PAVUK_CFGFNAME_PREFIX, PACKAGE);
    gtkrc[sizeof(gtkrc) - 1] = 0;
    gtk_rc_add_default_file(gtkrc);
  }
...
void *gui_tree_make_entry(url * urlp)
{
  ...
  if(urlp->status & URL_PROCESSED)
  {
    ...
    gtk_ctree_get_node_info(GTK_CTREE(gui_cfg.tree_widget), urlp->tree_nfo[0], &text, &sp, &p, &m, &pixmap, &shape, &il, &ex);
  }
  else
  {
  ...
  retv = gtk_ctree_insert_node(GTK_CTREE(gui_cfg.tree_widget), parent, NULL, text, 8, pixmap, shape, pixmap, shape, FALSE, TRUE);
...
void gui_create_tree_root_node(void)
{
  ...
  if(cfg.xi_face && !gui_cfg.root)
  {
    ...
    gui_cfg.root =
      ...
      gui_cfg.icon.notprocessed->shape, gui_cfg.icon.notprocessed->pixmap, gui_cfg.icon.notprocessed->shape, FALSE, TRUE);
    GDK_THREADS_LEAVE();
    UNLOCK_GTKTREE;
  }
#endif /* WITH_TREE */
...
int gui_wait_io(int sock, int for_read)
{
  ...
  if(cfg.conn_timeout > 0.0)
    inid = gtk_timeout_add((int) cfg.conn_timeout, (GtkFunction) TOut, &inid);

  if(cfg.conn_timeout > (double) ULONG_MAX)
    xprintf(0, gettext("Too high timeout value for GUI interface implementation of timeout\n"));

  iid = gdk_input_add(sock, for_read ? GDK_INPUT_READ : GDK_INPUT_WRITE, (GdkInputFunction) rwCB, (gpointer) & iid);
...
int gui_xprint(char *str)
{
  ...
  if(cfg.log_autoscroll)
  {
    gui_cfg.logvadj->value = gui_cfg.logvadj->upper >
      gui_cfg.logvadj->page_size ? gui_cfg.logvadj->upper - gui_cfg.logvadj->page_size : 0;
    gtk_signal_emit_by_name(GTK_OBJECT(gui_cfg.logvadj), "value_changed");
  }

#if 0                           /* [i_a] experminet while trying to track down the bug that causes the GTK crash :-( */
  {
    GtkCList *cl = GTK_CLIST(gui_cfg.logw);
#define CLIST_UNFROZEN(clist)     (((GtkCList*) (clist))->freeze_count == 0)
#define CLIST_REFRESH(clist)    { \
  if (CLIST_UNFROZEN (clist)) \
    GTK_CLIST_GET_CLASS (clist)->refresh ((GtkCList*) (clist)); \
}

    if(CLIST_UNFROZEN(cl))
    {
      GTK_CLIST_GET_CLASS(cl)->refresh((GtkCList *) (cl));
    }

    /* Need to refresh the widget --
       This forces the widget to redraw itself. */
    gtk_widget_queue_draw(GTK_WIDGET(cl));
  }
#endif
