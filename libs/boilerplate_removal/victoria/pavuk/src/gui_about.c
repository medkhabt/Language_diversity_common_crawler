...
void gui_build_about(int popup)
{
  ...
  gtk_window_set_title(GTK_WINDOW(gui_cfg.about_shell), gettext("Pavuk: About"));
  gtk_container_border_width(GTK_CONTAINER(gui_cfg.about_shell), 3);
  gtk_signal_connect(GTK_OBJECT(gui_cfg.about_shell), "destroy", GTK_SIGNAL_FUNC(gtk_widget_destroyed), &gui_cfg.about_shell);
  ...
  gtk_signal_connect(GTK_OBJECT(button), "clicked", GTK_SIGNAL_FUNC(guitl_PopdownW), (gpointer) gui_cfg.about_shell);
