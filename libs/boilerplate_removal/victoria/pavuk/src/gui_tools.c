...
GtkWidget *guitl_tab_add_entry(GtkWidget * table, char *label, guint col, guint row, guint passw)
{
  GtkWidget *entry, *labelw;

  labelw = gtk_label_new(label);
  gtk_misc_set_alignment(GTK_MISC(labelw), 0, 0.5);
  gtk_table_attach(GTK_TABLE(table), labelw, col, col + 1, row, row + 1, GTK_FILL | GTK_SHRINK, GTK_FILL, 2, 2);
  gtk_widget_show(labelw);

  entry = gtk_entry_new();
  gtk_table_attach(GTK_TABLE(table), entry, col + 1, col + 2, row, row + 1, GTK_FILL | GTK_EXPAND, GTK_FILL, 2, 2);
  if(passw)
...
GtkWidget *guitl_tab_add_numentry(GtkWidget * table, char *label, guint col, guint row, guint max_val)
{
  GtkWidget *entry, *labelw;
  GtkAdjustment *adj;
  GdkFont *font;
  ...
  adj = (GtkAdjustment *) gtk_adjustment_new(0.0, 0.0, (float) max_val, 1.0, 5.0, 0.0);

  entry = gtk_spin_button_new(adj, 0, 0);
  gtk_table_attach(GTK_TABLE(table), entry, col + 1, col + 2, row, row + 1, 0, GTK_FILL, 2, 2);
#if GTK_FACE < 2
...
GtkWidget *guitl_tab_add_doubleentry(GtkWidget * table, char *label, guint col, guint row, guint max_val, guint decimals)
{
  GtkWidget *entry, *labelw;
  GtkAdjustment *adj;
  GdkFont *font;
...
GtkWidget *guitl_tab_add_enum(GtkWidget * table, char *label, guint col, guint row, const char **vals, guint edit)
{
  GtkWidget *entry, *labelw;
  GList *ptr;
  int i;
...
static void edit_entry_ok(GtkWidget * w, struct edit_entry_info *info)
{
  ...
  for(i = 0; i < GTK_CLIST(info->list)->rows; i++)
  {
    ...
    if(info->quoted)
    {
      char *p1;
      p1 = escape_str(pom, "\"");
      p = tl_str_concat(DBGvars(p), p ? " " : "", "\"", p1, "\"", NULL);
      _free(p1);
    }
    else
    {
      p = tl_str_concat(DBGvars(p), p ? "," : "", pom, NULL);
    }
  }
...
static void edit_entry_start_edit(GtkWidget * w, struct edit_entry_info *info)
{
  ...
  if(!info->tl)
  {
    ...
    snprintf(pom, sizeof(pom), gettext("Pavuk: edit %s"), info->tlabel ? info->tlabel : gettext("entry"));
    pom[sizeof(pom) - 1] = 0;
    ...
    gtk_signal_connect(GTK_OBJECT(info->tl), "destroy", GTK_SIGNAL_FUNC(gtk_widget_destroyed), &(info->tl));
    ...
    brow = guitl_new_edit_list(&info->list, &info->list_entry, info->label, NULL, NULL, NULL, NULL, TRUE, NULL);
    ...
    gtk_signal_connect(GTK_OBJECT(button), "clicked", GTK_SIGNAL_FUNC(edit_entry_ok), (gpointer) info);
    ...
    gtk_container_add(GTK_CONTAINER(brow), button);
    gtk_signal_connect(GTK_OBJECT(button), "clicked", GTK_SIGNAL_FUNC(DestroyW), (gpointer) info->tl);
    GTK_WIDGET_SET_FLAGS(button, GTK_CAN_DEFAULT);
    gtk_widget_grab_default(button);
    gtk_widget_show(button);
  }
...
static void edit_entry_info_destroy(GtkWidget * w, struct edit_entry_info *info)
{
  if(info->tl)
  {
    gtk_signal_disconnect_by_func(GTK_OBJECT(info->tl), GTK_SIGNAL_FUNC(gtk_widget_destroyed), &(info->tl));
    gtk_widget_destroy(info->tl);
...
GtkWidget *guitl_tab_add_edit_entry(GtkWidget * table, char *label, char *elabel, guint col, guint row, guint quoted)
{
  ...
  info->tlabel = elabel ? tl_strdup(elabel) : (label ? tl_strndup(label, strlen(label) - 2) : NULL);
  info->label = tl_strdup(label);
  info->quoted = quoted;

  box = gtk_hbox_new(0, 0);
  gtk_widget_show(box);
  gtk_table_attach(GTK_TABLE(table), box, col, col + 1, row, row + 1, GTK_FILL | GTK_SHRINK, GTK_FILL, 2, 2);
  ...
  gtk_widget_show(pom);
  gtk_table_attach(GTK_TABLE(table), pom, col + ofs, col + ofs + 1, row, row + 1, GTK_FILL | GTK_EXPAND, GTK_FILL, 2, 2);

  info->entry = entry = gtk_entry_new();
  gtk_box_pack_start(GTK_BOX(pom), entry, TRUE, TRUE, 0);
  gtk_widget_show(entry);

  gtk_signal_connect(GTK_OBJECT(entry), "destroy", GTK_SIGNAL_FUNC(edit_entry_info_destroy), (gpointer) info);
  ...
  gtk_signal_connect(GTK_OBJECT(button), "clicked", GTK_SIGNAL_FUNC(edit_entry_start_edit), (gpointer) info);
...
static void SetFileEntry(GtkObject * object, struct path_entry_info *info)
{
  ...
  if(!info->isdir)
  {
    ...
    if(!tl_stat(p, &estat) && !S_ISREG(estat.st_mode))
      ok = FALSE;
  }
...
static void GetFile(GtkObject * object, struct path_entry_info *info)
{
  if(!info->fs)
  {
    ...
    snprintf(pom, sizeof(pom), gettext("Pavuk: select %s"), info->label ? info->label : gettext("file"));
    pom[sizeof(pom) - 1] = 0;
    ...
    gtk_signal_connect(GTK_OBJECT(info->fs), "destroy", GTK_SIGNAL_FUNC(gtk_widget_destroyed), &(info->fs));
    ...
    gtk_signal_connect(GTK_OBJECT(GTK_FILE_SELECTION(info->fs)->ok_button), "clicked", GTK_SIGNAL_FUNC(SetFileEntry), (gpointer) info);

    gtk_signal_connect(GTK_OBJECT(GTK_FILE_SELECTION(info->fs)->
        cancel_button), "clicked", GTK_SIGNAL_FUNC(DestroyW), (gpointer) info->fs);
  }
...
static void path_entry_info_destroy(GtkWidget * w, struct path_entry_info *info)
{
  if(info->fs)
  {
    gtk_signal_disconnect_by_func(GTK_OBJECT(info->fs), GTK_SIGNAL_FUNC(gtk_widget_destroyed), &(info->fs));
...
GtkWidget *guitl_tab_add_path_entry_full(GtkWidget * table, char *label, guint col, guint row, int dir, char *fslabel)
{
  GtkWidget *entry, *labelw, *pom, *button, *box;
  ...
  gtk_signal_connect(GTK_OBJECT(entry), "destroy", GTK_SIGNAL_FUNC(path_entry_info_destroy), (gpointer) info);
  ...
  gtk_signal_connect(GTK_OBJECT(button), "clicked", GTK_SIGNAL_FUNC(GetFile), (gpointer) info);

  return entry;
}

GtkWidget *guitl_tab_add_path_entry(GtkWidget * table, char *label, guint col, guint row, int dir)
{
...
void guitl_ListDeleteSelected(GtkObject * object, gpointer func_data)
{
  ...
  {
    gtk_clist_remove(GTK_CLIST(w), GPOINTER_TO_INT(GTK_CLIST(w)->selection->data));
  }
...
void guitl_ListCopyToEntry(GtkObject * object, int row, int col, GdkEvent * event, gpointer func_data)
{
...
void guitl_ListInsertList(GtkObject * object, gpointer func_data)
{
  ...
  while(dlist)
  {
    gtk_clist_get_text(GTK_CLIST(lnfo->entry), GPOINTER_TO_INT(dlist->data), 0, &p);
...
GtkWidget *guitl_new_edit_list(GtkWidget ** list, GtkWidget ** entry,
  ...
  GtkWidget ** cbutton, GtkWidget ** abutton, gboolean connect_entry, const char **vals)
{
  GtkWidget *swin;
  ...
  *list = gtk_clist_new(1);
  ...
  if(connect_entry)
    gtk_signal_connect(GTK_OBJECT(*list), "select_row", GTK_SIGNAL_FUNC(guitl_ListCopyToEntry), (gpointer) * entry);
  ...
  if(!abutton)
  {
    ...
    gtk_signal_connect(GTK_OBJECT(button), "clicked", GTK_SIGNAL_FUNC(guitl_ListInsertEntry), (gpointer) p);

    gtk_signal_connect(GTK_OBJECT(*entry), "activate", GTK_SIGNAL_FUNC(guitl_ListInsertEntry), (gpointer) p);
  }
  else
    *abutton = button;
  ...
  if(!mbutton)
  {
    gtk_signal_connect(GTK_OBJECT(button), "clicked", GTK_SIGNAL_FUNC(guitl_ListModifyEntry), (gpointer) p);
  }
  else
    *mbutton = button;
  ...
  if(!cbutton)
  {
    gtk_signal_connect(GTK_OBJECT(button), "clicked", GTK_SIGNAL_FUNC(guitl_ListClear), (gpointer) * list);
  }
  else
    *cbutton = button;
  ...
  if(!dbutton)
  {
    gtk_signal_connect(GTK_OBJECT(button), "clicked", GTK_SIGNAL_FUNC(guitl_ListDeleteSelected), (gpointer) * list);
  }
  else
    *dbutton = button;
...
GtkWidget *guitl_pixmap_button(char **pm, char *alticon, char *label)
{
  ...
  if(alticon)
  {
    ...
    pm = gdk_pixmap_create_from_xpm(GTK_WIDGET(gui_cfg.toplevel)->window, &mask, NULL, alticon);
...
{
  gtk_signal_connect(GTK_OBJECT(parent), "destroy", GTK_SIGNAL_FUNC(menu_kill), menu);
...
static unsigned char *guitl_selection_content = NULL;
enum
{
  GUITL_SEL_TYPE_NONE,
  GUITL_COMPOUND_TEXT,
...
static gint guitl_selection_clear(GtkWidget * widget, GdkEventSelection * event)
{
...
static void guitl_selection_get(GtkWidget * widget, GtkSelectionData * selection_data, guint info, guint time, gpointer data)
{
  ...
  gtk_selection_data_set(selection_data, type, 8 * sizeof(gchar), guitl_selection_content, strlen((char *) guitl_selection_content));
}
...
void guitl_set_clipboard_content(char *content)
{
  ...
  if(!initialized)
  {
    ...
    gtk_selection_add_targets(gui_cfg.toplevel, GDK_SELECTION_PRIMARY, targetlist, sizeof(targetlist) / sizeof(targetlist[0]));
    gtk_selection_add_targets(gui_cfg.toplevel, clipboard_atom, targetlist, sizeof(targetlist) / sizeof(targetlist[0]));
    gtk_signal_connect(GTK_OBJECT(gui_cfg.toplevel), "selection_get", GTK_SIGNAL_FUNC(guitl_selection_get), NULL);
    gtk_signal_connect(GTK_OBJECT(gui_cfg.toplevel), "selection_clear_event", GTK_SIGNAL_FUNC(guitl_selection_clear), NULL);
  }

  g_free(guitl_selection_content);
  guitl_selection_content = (unsigned char *) g_strdup(content);

  gtk_selection_owner_set(gui_cfg.toplevel, GDK_SELECTION_PRIMARY, GDK_CURRENT_TIME);
  clipboard_atom = gdk_atom_intern("CLIPBOARD", FALSE);
  ...
  char **pmap, GtkSignalFunc cb, gpointer cb_data, char *alticon)
{
  ...
  button = gtk_toolbar_append_item(GTK_TOOLBAR(parent), label, help, "", pixmap, cb, cb_data);
