...
js_transform_t *js_transform_new(const char *pattern, const char *transform, const char *tag, const char *attrib, int type)
{
  js_transform_t *rv;
  re_entry *re;
...
char *js_transform_apply(js_transform_t * jt, const char *attr, int nsub, int *subs)
{
  char *rv;
  int n, l;
  ...
  do
  {
    if(jt->transform[l] == '$')
    {
      ...
      if(n <= nsub)
      {
        rv = tl_str_nappend(rv, attr + subs[2 * n], subs[2 * n + 1] - subs[2 * n]);
      }
