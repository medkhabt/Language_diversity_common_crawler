...
void gui_set_debug_level_mi(void)
{
  ...
  for(i = 0; cfg_debug_levels[i].id; i++)
  {
    if(ch)
    {
      gtk_check_menu_item_set_state(GTK_CHECK_MENU_ITEM(ch->data), (cfg.debug_level & cfg_debug_levels[i].id) != 0);
      ch = ch->next;
    }
  }
}
...
static void Save_rc(GtkObject * object, gpointer func_data)
{
  ...
  pom[sizeof(pom) - 1] = 0;
  fd = bufio_open(pom, O_BINARY | O_WRONLY | O_CREAT);
  cfg_dump(&fd);
  ASSERT(fd ? !bufio_is_open(fd) : TRUE);
}
...
static void ChangeLang(GtkObject * object, gpointer func_data)
{
  ...
  {
    gdk_beep();
    xprintf(1, gettext("Unable to change language during processing time!\n"));
    return;
  }
...
void gui_window_drop_url(GtkWidget * widget, GdkDragContext * context,
  gint x, gint y, GtkSelectionData * seldata, guint info, guint time, gpointer data)
{
  gchar *drag_url;
  gchar *p_seldata;
  url_info *ui;
  ...
  if(gui_cfg.config_shell)
  {
    ...
    gtk_clist_set_row_data_full(GTK_CLIST(gui_cfg.url_list), row, cui, (GtkDestroyNotify) url_info_free);
  }
...
void gui_PopupW(GtkObject * object, gpointer func_data)
{
  ...
  case PAVUK_CFGSCH:
    gui_build_scheduler(TRUE);
    {
      ...
      gtk_calendar_select_month(GTK_CALENDAR(gui_cfg.calendar), ltime->tm_mon, ltime->tm_year);
      gtk_calendar_select_day(GTK_CALENDAR(gui_cfg.calendar), ltime->tm_mday);
      gtk_spin_button_set_value(GTK_SPIN_BUTTON(gui_cfg.hour_label), (gfloat) ltime->tm_hour);
      gtk_spin_button_set_value(GTK_SPIN_BUTTON(gui_cfg.min_label), (gfloat) ltime->tm_min);
...
static void FetchURL(GtkWidget * widget, GtkSelectionData * selection_data, gpointer data)
{
  char *p, *cb_url;
  url_info *ui;
...
static void FetchCBUrl(GtkObject * object, gpointer func_data)
{
  ...
  gtk_selection_convert(GTK_WIDGET(object), GDK_SELECTION_PRIMARY, atom, GDK_CURRENT_TIME);
}
...
static char *get_string_atom_value(Display * dpy, Window win, Atom atom)
{
  ...
  result = XGetWindowProperty(dpy, win, atom,
    0, (65536 / sizeof(long)), False, XA_STRING, &actual_type, &actual_format, &nitems, &bytes_after, &data);
...
static void fill_browser_menu(GtkWidget * menu)
{
  ...
  for(i = 0; i < num; i++)
  {
    ...
    if(u)
    {
      ...
      gtk_widget_show(mi);
      gtk_signal_connect(GTK_OBJECT(mi), "activate", GTK_SIGNAL_FUNC(AddBrowserURL), NULL);
...
static void StartRecurse(int restart)
{
...
#else
  {
    ...
    if(restart || cfg.mode != cfg.prev_mode || !cfg.mode_started)
    {
      if(pthread_create(&cfg.mainthread, &tattr, (void *(*)(void *)) absi_restart, NULL))
      {
    ...
    {
      if(pthread_create(&cfg.mainthread, &tattr, (void *(*)(void *)) absi_cont, NULL))
      {
        xperror("Create main thread");
        gui_finish_download(TRUE);
      }
...
} tbar_setup_rec[] =
{
  ...
  {
  gettext_nop("Icons only"), "config/tb_icons", "tb_icons_e", GTK_TOOLBAR_ICONS},
  {
...
} statusbar_col_rec[] =
{
  ...
  {
  gettext_nop("URL"), "config/sb_url", "status_col_url_e", "status_col_url_width", 150},
  {
  gettext_nop("Status"), "config/sb_status", "status_col_status_e", "status_col_status_width", 150},
  {
  gettext_nop("Size"), "config/sb_size", "status_col_size_e", "status_col_size_width", 150},
  {
  gettext_nop("Transfer rate"), "config/sb_rate", "status_col_rate_e", "status_col_rate_width", 60},
  {
  gettext_nop("Elapsed time"), "config/sb_et", "status_col_et_e", "status_col_et_width", 80},
  {
  gettext_nop("Remaining time"), "config/sb_rt", "status_col_rt_e", "status_col_rt_width", 80}
};

static void statusbar_col_set(GtkObject * object, gpointer func_data)
{
  gtk_clist_set_column_visibility(GTK_CLIST(gui_cfg.status_list), (int) func_data, GTK_CHECK_MENU_ITEM(object)->active);

  gprop_set_bool_t(statusbar_col_rec[(int) func_data].prope, GTK_CHECK_MENU_ITEM(object)->active);
}
#endif
...
static void toolbar_set(GtkObject * object, gpointer func_data)
{
  ...
  {
    gtk_toolbar_set_style(GTK_TOOLBAR(gui_cfg.toolbar), tbar_setup_rec[(int) func_data].tbtype);
...
static void build_menu(GtkWidget * parent)
{
  ...
  gtk_signal_connect(GTK_OBJECT(button), "clicked", GTK_SIGNAL_FUNC(Miniaturize), (gpointer) parent);
...
/*** FILE MENU ***/
  ...
  gtk_signal_connect(GTK_OBJECT(mi), "activate", GTK_SIGNAL_FUNC(gui_PopupW), (gpointer) PAVUK_CFGCOMM);

  gtk_signal_connect(GTK_OBJECT(mi), "activate", GTK_SIGNAL_FUNC(OpenURL), (gpointer) NULL);
  ...
  gtk_signal_connect(GTK_OBJECT(mi), "activate", GTK_SIGNAL_FUNC(gui_build_addurl), (gpointer) NULL);

  mi = menu_item(gettext("Fetch URL from Clipboard"));
  gaccel_bind_widget("file/fetch_url", "activate", mi, NULL, gui_cfg.toplevel);
  gtk_menu_append(GTK_MENU(menu), mi);
  gtk_widget_show(mi);

  gtk_signal_connect(GTK_OBJECT(mi), "activate", GTK_SIGNAL_FUNC(FetchCBUrl), (gpointer) NULL);
  gtk_signal_connect(GTK_OBJECT(mi), "selection_received", GTK_SIGNAL_FUNC(FetchURL), (gpointer) NULL);
#ifdef HAVE_XMU
  smenu = gtk_menu_new();
  gtk_widget_realize(smenu);
  gtk_signal_connect(GTK_OBJECT(smenu), "show", GTK_SIGNAL_FUNC(BrowserMenuPopup), (gpointer) NULL);

  mi = menu_item(gettext("Fetch URL from browser"));
  gaccel_bind_widget("file/browser_url", "activate", mi, NULL, gui_cfg.toplevel);
  ...
  gtk_signal_connect(GTK_OBJECT(mi), "activate", GTK_SIGNAL_FUNC(gui_PopupW), (gpointer) PAVUK_SCNLD);
  ...
  gtk_signal_connect(GTK_OBJECT(mi), "activate", GTK_SIGNAL_FUNC(gui_PopupW), (gpointer) PAVUK_SCNADD);
  ...
  gtk_signal_connect(GTK_OBJECT(mi), "activate", GTK_SIGNAL_FUNC(gui_PopupW), (gpointer) PAVUK_SCNSV);

  mi = menu_item(gettext("Save settings to ~/" PAVUK_PREFERENCES_FILENAME));
  gaccel_bind_widget("file/save_pavukrc", "activate", mi, NULL, gui_cfg.toplevel);
  gtk_menu_append(GTK_MENU(menu), mi);
  gtk_widget_show(mi);

  gtk_signal_connect(GTK_OBJECT(mi), "activate", GTK_SIGNAL_FUNC(Save_rc), (gpointer) NULL);
  ...
  gtk_signal_connect(GTK_OBJECT(mi), "activate", GTK_SIGNAL_FUNC(gui_PopupW), (gpointer) PAVUK_CFGSCH);

  mi = menu_item(gettext("Auth. info editor ..."));
  gaccel_bind_widget("file/auth_edit", "activate", mi, NULL, gui_cfg.toplevel);
  gtk_menu_append(GTK_MENU(menu), mi);
  gtk_widget_show(mi);

  gtk_signal_connect(GTK_OBJECT(mi), "activate", GTK_SIGNAL_FUNC(gauthinfo_run), (gpointer) NULL);
  ...
  gtk_signal_connect(GTK_OBJECT(mi), "activate", GTK_SIGNAL_FUNC(Quit), (gpointer) NULL);
...
/*** VIEW MENU ***/
  ...
  gtk_signal_connect(GTK_OBJECT(mi), "activate", GTK_SIGNAL_FUNC(gui_PopupW), (gpointer) PAVUK_TREE);
#endif

  mi = menu_item(gettext("Status page ..."));
  gaccel_bind_widget("view/status_page", "activate", mi, NULL, gui_cfg.toplevel);
  gtk_menu_append(GTK_MENU(menu), mi);
  gtk_widget_show(mi);

  gtk_signal_connect(GTK_OBJECT(mi), "activate", GTK_SIGNAL_FUNC(stats_show), (gpointer) NULL);

  mi = menu_item(gettext("HTML forms editor ..."));
  gaccel_bind_widget("view/form_editor", "activate", mi, NULL, gui_cfg.toplevel);
  gtk_menu_append(GTK_MENU(menu), mi);
  gtk_widget_show(mi);

  gtk_signal_connect(GTK_OBJECT(mi), "activate", GTK_SIGNAL_FUNC(form_edit_dlg), (gpointer) NULL);

#ifdef HAVE_MOZJS
  mi = menu_item(gettext("Javascript console ..."));
  gaccel_bind_widget("view/js_console", "activate", mi, NULL, gui_cfg.toplevel);
  gtk_menu_append(GTK_MENU(menu), mi);
  gtk_widget_show(mi);

  gtk_signal_connect(GTK_OBJECT(mi), "activate", GTK_SIGNAL_FUNC(gui_PopupW), (gpointer) PAVUK_JSCONS);
#endif
  ...
  gaccel_bind_widget("view/clear_log", "activate", mi, NULL, gui_cfg.toplevel);
  gtk_menu_append(GTK_MENU(menu), mi);
  gtk_widget_show(mi);

  gtk_signal_connect(GTK_OBJECT(mi), "activate", GTK_SIGNAL_FUNC(ClearLog), (gpointer) NULL);
...
/*** MODE MENU ***/
  ...
  gaccel_bind_widget("mode/normal_recurse", "activate", mi, NULL, gui_cfg.toplevel);
  rg = gtk_radio_menu_item_group(GTK_RADIO_MENU_ITEM(mi));
  gtk_menu_append(GTK_MENU(menu), mi);
  gtk_widget_show(mi);
  gtk_signal_connect(GTK_OBJECT(mi), "toggled", GTK_SIGNAL_FUNC(SwitchMode), (gpointer) MODE_NORMAL);
  gui_cfg.modegr[MODE_NORMAL] = mi;

  mi = gtk_radio_menu_item_new_with_label(rg, gettext("synchronize"));
  gaccel_bind_widget("mode/synchronize", "activate", mi, NULL, gui_cfg.toplevel);
  rg = gtk_radio_menu_item_group(GTK_RADIO_MENU_ITEM(mi));
  gtk_menu_append(GTK_MENU(menu), mi);
  gtk_widget_show(mi);
  gtk_signal_connect(GTK_OBJECT(mi), "toggled", GTK_SIGNAL_FUNC(SwitchMode), (gpointer) MODE_SYNC);
  ...
  gtk_menu_append(GTK_MENU(menu), mi);
  gtk_widget_show(mi);
  gtk_signal_connect(GTK_OBJECT(mi), "toggled", GTK_SIGNAL_FUNC(SwitchMode), (gpointer) MODE_MIRROR);
  gui_cfg.modegr[MODE_MIRROR] = mi;

  mi = gtk_radio_menu_item_new_with_label(rg, gettext("single page"));
  gaccel_bind_widget("mode/single_page", "activate", mi, NULL, gui_cfg.toplevel);
  rg = gtk_radio_menu_item_group(GTK_RADIO_MENU_ITEM(mi));
  gtk_menu_append(GTK_MENU(menu), mi);
  gtk_widget_show(mi);
  gtk_signal_connect(GTK_OBJECT(mi), "activate", GTK_SIGNAL_FUNC(SwitchMode), (gpointer) MODE_SINGLE);
  gui_cfg.modegr[MODE_SINGLE] = mi;

  mi = gtk_radio_menu_item_new_with_label(rg, gettext("update local links"));
  gaccel_bind_widget("mode/update_local", "activate", mi, NULL, gui_cfg.toplevel);
  rg = gtk_radio_menu_item_group(GTK_RADIO_MENU_ITEM(mi));
  gtk_menu_append(GTK_MENU(menu), mi);
  gtk_widget_show(mi);
  gtk_signal_connect(GTK_OBJECT(mi), "toggled", GTK_SIGNAL_FUNC(SwitchMode), (gpointer) MODE_LNUPD);
  gui_cfg.modegr[MODE_LNUPD] = mi;

  mi = gtk_radio_menu_item_new_with_label(rg, gettext("resume files"));
  gaccel_bind_widget("mode/resume_files", "activate", mi, NULL, gui_cfg.toplevel);
  rg = gtk_radio_menu_item_group(GTK_RADIO_MENU_ITEM(mi));
  gtk_menu_append(GTK_MENU(menu), mi);
  gtk_widget_show(mi);
  gtk_signal_connect(GTK_OBJECT(mi), "toggled", GTK_SIGNAL_FUNC(SwitchMode), (gpointer) MODE_RESUME);
  gui_cfg.modegr[MODE_RESUME] = mi;

  mi = gtk_radio_menu_item_new_with_label(rg, gettext("unlimited reget"));
  gaccel_bind_widget("mode/unlimited_reget", "activate", mi, NULL, gui_cfg.toplevel);
  rg = gtk_radio_menu_item_group(GTK_RADIO_MENU_ITEM(mi));
  gtk_menu_append(GTK_MENU(menu), mi);
  gtk_widget_show(mi);
  gtk_signal_connect(GTK_OBJECT(mi), "toggled", GTK_SIGNAL_FUNC(SwitchMode), (gpointer) MODE_SREGET);
  ...
  gtk_menu_append(GTK_MENU(menu), mi);
  gtk_widget_show(mi);
  gtk_signal_connect(GTK_OBJECT(mi), "toggled", GTK_SIGNAL_FUNC(SwitchMode), (gpointer) MODE_NOSTORE);
  ...
  gtk_menu_append(GTK_MENU(menu), mi);
  gtk_widget_show(mi);
  gtk_signal_connect(GTK_OBJECT(mi), "toggled", GTK_SIGNAL_FUNC(SwitchMode), (gpointer) MODE_REMIND);
  ...
  gtk_menu_append(GTK_MENU(menu), mi);
  gtk_widget_show(mi);
  gtk_signal_connect(GTK_OBJECT(mi), "toggled", GTK_SIGNAL_FUNC(SwitchMode), (gpointer) MODE_FTPDIR);
  ...
  gtk_check_menu_item_set_state(GTK_CHECK_MENU_ITEM(gui_cfg.modegr[cfg.mode]), TRUE);
...
/*** CONFIG MENU ***/
  ...
  gaccel_bind_widget("config/common_cfg", "activate", mi, NULL, gui_cfg.toplevel);
  gtk_menu_append(GTK_MENU(menu), mi);
  gtk_widget_show(mi);
  ...
  gaccel_bind_widget("config/limit_cfg", "activate", mi, NULL, gui_cfg.toplevel);
  gtk_menu_append(GTK_MENU(menu), mi);
  gtk_widget_show(mi);

  gtk_signal_connect(GTK_OBJECT(mi), "activate", GTK_SIGNAL_FUNC(gui_PopupW), (gpointer) PAVUK_CFGLIM);

  mi = menu_item(gettext("Reset configuration"));
  gaccel_bind_widget("config/reset_cfg", "activate", mi, NULL, gui_cfg.toplevel);
  gtk_menu_append(GTK_MENU(menu), mi);
  gtk_widget_show(mi);

  gtk_signal_connect(GTK_OBJECT(mi), "activate", GTK_SIGNAL_FUNC(ResetCfg), (gpointer) NULL);
...
/*** toolbar menu ***/
  ...
  gaccel_bind_widget("config/tb_toggle", "activate", mi, NULL, gui_cfg.toplevel);
  gtk_menu_append(GTK_MENU(smenu), mi);
  gtk_widget_show(mi);
  gtk_signal_connect(GTK_OBJECT(mi), "activate", (GtkSignalFunc) toolbar_onoff, (gpointer) NULL);
...
#define TBSET_ME(label, tb_style)\
  ...
  for(i = 0; i < (sizeof(tbar_setup_rec) / sizeof(tbar_setup_rec[0])); i++)
  {
    ...
    gaccel_bind_widget(tbar_setup_rec[i].accel, "activate", mi, NULL, gui_cfg.toplevel);
...
#define SBSET_ME(label, cn)\
  ...
  for(i = 1; i < (sizeof(statusbar_col_rec) / sizeof(statusbar_col_rec[0])); i++)
  {
    int rv;

    SBSET_ME(statusbar_col_rec[i].label, i);
    gaccel_bind_widget(statusbar_col_rec[i].accel, "activate", mi, NULL, gui_cfg.toplevel);
...
/*** status bar menu end ***/
...
#ifdef GETTEXT_NLS
  {
    ...
    if(al || (al = get_available_languages()))
    {
      ...
      gtk_signal_connect(GTK_OBJECT(mi), "activate", GTK_SIGNAL_FUNC(ChangeLang), (gpointer) "C");
      ...
      for(i = 0; al[i]; i++)
      {
        ...
        gtk_signal_connect(GTK_OBJECT(mi), "activate", GTK_SIGNAL_FUNC(ChangeLang), (gpointer) al[i]);
      }
    }
  }
#endif
  ...
  for(i = 0; cfg_debug_levels[i].id; i++)
  {
    if(!cfg_debug_levels[i].label)
      continue;
    mi = gtk_check_menu_item_new_with_label(gettext(cfg_debug_levels[i].label));
    gtk_menu_append(GTK_MENU(gui_cfg.debug_level_m), mi);
    gtk_widget_show(mi);
    gtk_signal_connect(GTK_OBJECT(mi), "activate", GTK_SIGNAL_FUNC(switch_debug_level), (gpointer) cfg_debug_levels[i].id);
  }
  gui_set_debug_level_mi();
  ...
  gtk_menu_append(GTK_MENU(gui_cfg.debug_level_m), mi);
  gtk_widget_show(mi);
  gtk_signal_connect(GTK_OBJECT(mi), "activate", GTK_SIGNAL_FUNC(set_debug_level), (gpointer) 0xffffffff);

  mi = gtk_menu_item_new_with_label(gettext("None"));
  gtk_menu_append(GTK_MENU(gui_cfg.debug_level_m), mi);
  gtk_widget_show(mi);
  gtk_signal_connect(GTK_OBJECT(mi), "activate", GTK_SIGNAL_FUNC(set_debug_level), (gpointer) 0);

  gui_cfg.me_debug = mi = gtk_check_menu_item_new_with_label(gettext("Debug"));
  gaccel_bind_widget("config/debug", "activate", mi, NULL, gui_cfg.toplevel);
  gtk_menu_append(GTK_MENU(menu), mi);
  gtk_widget_show(mi);

  gtk_signal_connect(GTK_OBJECT(mi), "toggled", GTK_SIGNAL_FUNC(toggle_debug), (gpointer) & cfg.debug);
  ...
  gaccel_bind_widget("config/tooltips", "activate", mi, NULL, gui_cfg.toplevel);
  gtk_menu_append(GTK_MENU(menu), mi);
  gtk_widget_show(mi);
  gtk_check_menu_item_set_state(GTK_CHECK_MENU_ITEM(mi), !gprop_get_bool_t("tooltips_enabled", &i) || i);

  gtk_signal_connect(GTK_OBJECT(mi), "toggled", GTK_SIGNAL_FUNC(ToggleTooltips), NULL);

  mi = gtk_check_menu_item_new_with_label(gettext("Log window autoscroll"));
  gaccel_bind_widget("config/log_sroll", "activate", mi, NULL, gui_cfg.toplevel);
  gtk_menu_append(GTK_MENU(menu), mi);
  gtk_widget_show(mi);
  gtk_check_menu_item_set_state(GTK_CHECK_MENU_ITEM(mi), cfg.log_autoscroll);
  gtk_signal_connect(GTK_OBJECT(mi), "toggled", GTK_SIGNAL_FUNC(ToggleBool), (gpointer) & cfg.log_autoscroll);

  mi = gtk_check_menu_item_new_with_label(gettext("Use preferences"));
  gaccel_bind_widget("config/use_pref", "activate", mi, NULL, gui_cfg.toplevel);
  gtk_menu_append(GTK_MENU(menu), mi);
  gtk_widget_show(mi);
  gtk_check_menu_item_set_state(GTK_CHECK_MENU_ITEM(mi), cfg.use_prefs);
  gtk_signal_connect(GTK_OBJECT(mi), "toggled", GTK_SIGNAL_FUNC(ToggleBool), (gpointer) & cfg.use_prefs);

  gui_cfg.me_quiet = mi = gtk_check_menu_item_new_with_label(gettext("Quiet"));
  ...
  gtk_signal_connect(GTK_OBJECT(mi), "toggled", GTK_SIGNAL_FUNC(ToggleBool), (gpointer) & cfg.quiet);

#ifdef HAVE_MT
  gui_cfg.immessages = mi = gtk_check_menu_item_new_with_label(gettext("Immediate messages"));
  gaccel_bind_widget("config/immediate_msg", "activate", mi, NULL, gui_cfg.toplevel);
  gtk_menu_append(GTK_MENU(menu), mi);
  gtk_widget_show(mi);

  gtk_check_menu_item_set_state(GTK_CHECK_MENU_ITEM(mi), cfg.immessages);

  gtk_signal_connect(GTK_OBJECT(mi), "toggled", GTK_SIGNAL_FUNC(ToggleBool), (gpointer) & cfg.immessages);
#endif
...
/*** ACTION MENU ***/
  ...
  gaccel_bind_widget("action/restart", "activate", mi, NULL, gui_cfg.toplevel);
  gtk_menu_append(GTK_MENU(menu), mi);
  gtk_widget_show(mi);
  gui_cfg.mea_rest = mi;

  gtk_signal_connect(GTK_OBJECT(mi), "activate", GTK_SIGNAL_FUNC(Start), (gpointer) TRUE);

  mi = menu_item(gettext("Co_ntinue"));
  gaccel_bind_widget("action/continue", "activate", mi, NULL, gui_cfg.toplevel);
  gtk_menu_append(GTK_MENU(menu), mi);
  gtk_widget_show(mi);
  gui_cfg.mea_start = mi;

  gtk_signal_connect(GTK_OBJECT(mi), "activate", GTK_SIGNAL_FUNC(Start), (gpointer) FALSE);
  ...
  gtk_signal_connect(GTK_OBJECT(mi), "activate", GTK_SIGNAL_FUNC(Stop), (gpointer) NULL);
  ...
  gtk_signal_connect(GTK_OBJECT(mi), "activate", GTK_SIGNAL_FUNC(Break), (gpointer) NULL);
...
/*** HELP MENU ***/
  ...
  gtk_signal_connect(GTK_OBJECT(mi), "activate", GTK_SIGNAL_FUNC(gui_PopupW), (gpointer) PAVUK_ABOUT);
...
static void build_toolbar(GtkWidget * parent)
{
...
#if GTK_FACE < 2
  gui_cfg.toolbar = row = gtk_toolbar_new(GTK_ORIENTATION_HORIZONTAL, GTK_TOOLBAR_BOTH);
  ...
  gtk_toolbar_set_orientation(GTK_TOOLBAR(gui_cfg.toolbar), GTK_ORIENTATION_HORIZONTAL);
  ...
  gui_cfg.bt_cfg = guitl_toolbar_button(row, gettext("Config"),
    gettext("Popup config window"), configure_xpm, GTK_SIGNAL_FUNC(gui_PopupW), (gpointer) PAVUK_CFGCOMM, cfg.bt_icon_cfg);

  gui_cfg.bt_lim = guitl_toolbar_button(row, gettext("Limits"),
    gettext("Popup limits window"), limits_xpm, GTK_SIGNAL_FUNC(gui_PopupW), (gpointer) PAVUK_CFGLIM, cfg.bt_icon_lim);

  gtk_toolbar_append_space(GTK_TOOLBAR(row));

  gui_cfg.bt_bg = guitl_toolbar_button(row, gettext("Go bg"),
    gettext("Destroy window as soon as posible and continue on terminal"), gobg_xpm, GTK_SIGNAL_FUNC(Gobg), NULL, cfg.bt_icon_gobg);

  gui_cfg.bt_rest = guitl_toolbar_button(row, gettext("Restart"),
    gettext("Start working on currently set starting URLs"), restart_xpm, GTK_SIGNAL_FUNC(Start), (gpointer) TRUE, cfg.bt_icon_rest);

  gui_cfg.bt_start = guitl_toolbar_button(row, gettext("Continue"),
    gettext("Continue after stop or break"), continue_xpm, GTK_SIGNAL_FUNC(Start), (gpointer) FALSE, cfg.bt_icon_cont);

  gtk_toolbar_append_space(GTK_TOOLBAR(row));

  gui_cfg.bt_stop = guitl_toolbar_button(row, gettext("Stop"),
    gettext("Finish this transfer and stop"), stop_xpm, GTK_SIGNAL_FUNC(Stop), (gpointer) NULL, cfg.bt_icon_stop);

  gui_cfg.bt_break = guitl_toolbar_button(row, gettext("Break"),
    gettext("Break transfer and stop"), break_xpm, GTK_SIGNAL_FUNC(Break), (gpointer) NULL, cfg.bt_icon_brk);

  gtk_toolbar_append_space(GTK_TOOLBAR(row));

  gui_cfg.bt_exit = guitl_toolbar_button(row, gettext("Exit"),
    gettext("Immediately quit the program"), exit_xpm, GTK_SIGNAL_FUNC(Quit), (gpointer) NULL, cfg.bt_icon_exit);
...
static void build_minitoolbar(GtkWidget * col)
{
  ...
  gui_cfg.mtb_rest = guitl_toolbar_button(tb, NULL,
    gettext("Restart"), restart_small_xpm, GTK_SIGNAL_FUNC(Start), (gpointer) TRUE, cfg.bt_icon_rest_s);

  gui_cfg.mtb_start = guitl_toolbar_button(tb, NULL,
    gettext("Continue"), continue_small_xpm, GTK_SIGNAL_FUNC(Start), (gpointer) FALSE, cfg.bt_icon_cont_s);

  gui_cfg.mtb_stop = guitl_toolbar_button(tb, NULL,
    gettext("Stop"), stop_small_xpm, GTK_SIGNAL_FUNC(Stop), (gpointer) NULL, cfg.bt_icon_stop_s);

  gui_cfg.mtb_break = guitl_toolbar_button(tb, NULL,
    gettext("Break"), break_small_xpm, GTK_SIGNAL_FUNC(Break), (gpointer) NULL, cfg.bt_icon_brk_s);

  guitl_toolbar_button(tb, NULL,
    gettext("Show whole main window"), maximize_small_xpm, GTK_SIGNAL_FUNC(DeMiniaturize), (gpointer) col, cfg.bt_icon_mtb_s);

  guitl_toolbar_button(tb, NULL, gettext("Quit"), close_small_xpm, GTK_SIGNAL_FUNC(Quit), (gpointer) NULL, cfg.bt_icon_exit_s);
...
static guint gui_logw_list_events(GtkWidget * widget, GdkEvent * event)
{
  ...
  case GDK_BUTTON_PRESS:
    ...
    if(bevent->button == 3)
    {
      gtk_widget_set_sensitive(gui_cfg.logw_copy_me, (GTK_CLIST(gui_cfg.logw)->selection != NULL));
      gtk_menu_popup(GTK_MENU(gui_cfg.logw_menu), NULL, NULL, NULL, NULL, 3, bevent->time);
    }
    break;
  default:
    break;
...
static void logw_save_content_do(GtkWidget * w, GtkWidget * fs)
{
  ...
  for(row = 0; row < GTK_CLIST(gui_cfg.logw)->rows; row++)
  {
    ...
    t = tl_str_concat(DBGvars(t), s, "\n", NULL);
  }
  UNLOCK_GTKLOG;

  p = gtk_file_selection_get_filename(GTK_FILE_SELECTION(fs));

  if((fd = tl_open(p, O_BINARY | O_WRONLY | O_CREAT | O_TRUNC, S_IRUSR | S_IWUSR | S_IRGRP | S_IROTH)) < 0)
  {
    xperror(mk_native(p));
    gdk_beep();
    err = TRUE;
...
static void logw_save_content(void)
{
  ...
  if(!fs)
  {
    ...
    gtk_signal_connect(GTK_OBJECT(GTK_FILE_SELECTION(fs)->ok_button), "clicked", GTK_SIGNAL_FUNC(logw_save_content_do), fs);
    gtk_signal_connect(GTK_OBJECT(GTK_FILE_SELECTION(fs)->cancel_button), "clicked", GTK_SIGNAL_FUNC(guitl_PopdownW), fs);
...
static void build_logw(GtkWidget * pane)
{
  ...
  gtk_clist_set_selection_mode(GTK_CLIST(gui_cfg.logw), GTK_SELECTION_EXTENDED);
  ...
  gtk_scrolled_window_set_policy(GTK_SCROLLED_WINDOW(swin), GTK_POLICY_ALWAYS, GTK_POLICY_ALWAYS);
  gtk_box_pack_start(GTK_BOX(par), swin, TRUE, TRUE, 0);
  gtk_widget_show(swin);
  gtk_container_add(GTK_CONTAINER(swin), gui_cfg.logw);
  gui_cfg.logvadj = gtk_scrolled_window_get_vadjustment(GTK_SCROLLED_WINDOW(swin));
  gtk_widget_show(gui_cfg.logw);
  UNLOCK_GTKLOG;

  gtk_signal_connect(GTK_OBJECT(gui_cfg.logw), "event", GTK_SIGNAL_FUNC(gui_logw_list_events), NULL);

  gui_cfg.logw_menu = gtk_menu_new();
  gtk_widget_realize(gui_cfg.logw_menu);

  mi = menu_item(gettext("Select all"));
  gaccel_bind_widget("logw/select_all", "activate", mi, NULL, gui_cfg.toplevel);
  gtk_menu_append(GTK_MENU(gui_cfg.logw_menu), mi);
  gtk_widget_show(mi);
  gtk_signal_connect(GTK_OBJECT(mi), "activate", GTK_SIGNAL_FUNC(logw_select_all), gui_cfg.logw);

  mi = menu_item(gettext("Clear selection"));
  gaccel_bind_widget("logw/clear_selection", "activate", mi, NULL, gui_cfg.toplevel);
  gtk_menu_append(GTK_MENU(gui_cfg.logw_menu), mi);
  gtk_widget_show(mi);
  gtk_signal_connect(GTK_OBJECT(mi), "activate", GTK_SIGNAL_FUNC(logw_clear_selection), gui_cfg.logw);

  gui_cfg.logw_copy_me = mi = menu_item(gettext("Copy selection"));
  gaccel_bind_widget("logw/copy", "activate", mi, NULL, gui_cfg.toplevel);
  gtk_menu_append(GTK_MENU(gui_cfg.logw_menu), mi);
  gtk_widget_show(mi);
  gtk_signal_connect(GTK_OBJECT(mi), "activate", GTK_SIGNAL_FUNC(guitl_clist_selection_to_clipboard), gui_cfg.logw);
  ...
  gtk_menu_append(GTK_MENU(gui_cfg.logw_menu), mi);
  gtk_widget_show(mi);
  gtk_signal_connect(GTK_OBJECT(mi), "activate", GTK_SIGNAL_FUNC(logw_save_content), NULL);

  mi = menu_item(gettext("Clear log"));
  gaccel_bind_widget("logw/clear", "activate", mi, NULL, gui_cfg.toplevel);
  gtk_menu_append(GTK_MENU(gui_cfg.logw_menu), mi);
  gtk_widget_show(mi);
  gtk_signal_connect(GTK_OBJECT(mi), "activate", GTK_SIGNAL_FUNC(ClearLog), NULL);
}


#ifdef HAVE_MT
static void status_list_resize_column(GtkWidget * w, int col, int width, gpointer fdata)
{
...
static void build_statusbar(GtkWidget * pane)
{
  ...
  gtk_signal_connect(GTK_OBJECT(gui_cfg.status_list), "resize_column", GTK_SIGNAL_FUNC(status_list_resize_column), NULL);
  for(i = 0; i < (sizeof(statusbar_col_rec) / sizeof(statusbar_col_rec[0])); i++)
  {
    int w;

    gtk_clist_set_column_title(GTK_CLIST(gui_cfg.status_list), i, gettext(statusbar_col_rec[i].label));
...
static void toplevel_resize(GtkWidget * w, GtkAllocation * sallocation, gpointer fdata)
{
  char pom[64];
...
#ifdef HAVE_MT
static void pane_resize(GtkWidget * w, GtkAllocation * sallocation, gpointer fdata)
{
...
static void build_main(void)
{
  ...
  /*** DRAG'N'DROP ***/
  gtk_drag_dest_set(gui_cfg.toplevel, GTK_DEST_DEFAULT_ALL, dragtypes, NUM_ELEM(dragtypes), GDK_ACTION_COPY | GDK_ACTION_MOVE);
  gtk_signal_connect(GTK_OBJECT(gui_cfg.toplevel), "drag_data_received", GTK_SIGNAL_FUNC(gui_window_drop_url), NULL);

  gtk_signal_connect(GTK_OBJECT(gui_cfg.toplevel), "destroy", GTK_SIGNAL_FUNC(Quit), NULL);
  gtk_signal_connect(GTK_OBJECT(gui_cfg.toplevel), "size_allocate", GTK_SIGNAL_FUNC(toplevel_resize), NULL);
...
#ifdef WITH_TREE
  gui_build_tree_preview(gui_cfg.tree_shell ? GTK_WIDGET_VISIBLE(gui_cfg.tree_shell) : FALSE);
#endif
  ...
  gtk_signal_connect(GTK_OBJECT(GTK_PANED(logw_pane)->child1), "size_allocate", GTK_SIGNAL_FUNC(pane_resize), NULL);
#else
  ...
  gtk_signal_connect(GTK_OBJECT(gui_cfg.toplevel), "destroy", GTK_SIGNAL_FUNC(gtk_widget_destroyed), &gui_cfg.toplevel);
  ...
  SendMessage(GDK_WINDOW_XWINDOW(gui_cfg.toplevel->window),
    ...
    (struct _HICON *) LoadImage(GetModuleHandle(NULL),
      MAKEINTRESOURCE(PAVUK_ICON), IMAGE_ICON, GetSystemMetrics(SM_CXSMICON), GetSystemMetrics(SM_CYSMICON), 0));
  SendMessage(GDK_WINDOW_XWINDOW(gui_cfg.toplevel->window),
    WM_SETICON, ICON_BIG,
    (struct _HICON *) LoadImage(GetModuleHandle(NULL),
      MAKEINTRESOURCE(PAVUK_ICON), IMAGE_ICON, GetSystemMetrics(SM_CXSMICON), GetSystemMetrics(SM_CYSMICON), 0));
