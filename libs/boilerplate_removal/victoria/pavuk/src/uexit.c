...
int uexit_condition(url * urlp, int *size, time_t date)
{
  ...
  cmd = tl_str_concat(DBGvars(NULL), priv_cfg.condition.uexit, " -url '", urlstr, "'", NULL);
  _free(urlstr);
  ...
  for(ptr = urlp->parent_url; ptr; ptr = ptr->next)
  {
    ...
    cmd = tl_str_concat(DBGvars(cmd), " -parent '", urlstr, "'", NULL);
    _free(urlstr);
  }
  UNLOCK_URL(urlp);

  sprintf(pom, "-level %hu ", urlp->level);
  cmd = tl_str_concat(DBGvars(cmd), pom, NULL);
...
int uexit_follow_cmd(doc * docp)
{
  ...
  if(!cfg.no_disc_io)
  {
    ...
    cmd = tl_str_concat(DBGvars(NULL), priv_cfg.condition.follow_cmd, " -url '", p, "'", NULL);
    _free(p);

    p = url_to_in_filename(docp->doc_url);
    cmd = tl_str_concat(DBGvars(cmd), " -infile '", p, "'", NULL);
    _free(p);
