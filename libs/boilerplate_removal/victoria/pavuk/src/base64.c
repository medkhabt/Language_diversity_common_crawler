...
static const char base64_etab[] = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/";
...
char *base64_encode_data(const void *data, int len)
{
  ...
  while(idx < len)
  {
    ...
    {
      outstr[outidx] = base64_etab[(int) (((instr[idx - 1] << 4) & 0x30) | ((instr[idx] >> 4) & 0x0f))];
      outidx++;
...
int base64_decode_data(const char *inbuf, void **outbuf)
{
