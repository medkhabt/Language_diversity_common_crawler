...
bufio *get_file_data_socket(doc * docp)
{
  ...
  if(tl_stat(docp->doc_url->p.file.filename, &estat) == 0)
  {
  ...
  if(!(docp->datasock = bufio_open(docp->doc_url->p.file.filename, O_BINARY | O_RDONLY)))
  {
    docp->errcode = ERR_FILE_OPEN;
    xperror(mk_native(docp->doc_url->p.file.filename));
  }
