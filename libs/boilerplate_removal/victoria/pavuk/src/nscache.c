...
static void ns_cache_open_db(char *dirname)
{
  ...
  if(dirname)
  {
    ...
    snprintf(pom, sizeof(pom), "%s%sindex.db", dirname, (tl_is_dirname(dirname) ? "" : "/"));
    pom[sizeof(pom) - 1] = 0;

    if(!ns_cache_filename || strcmp(pom, ns_cache_filename))
      reopen = TRUE;

    if(!reopen && !tl_stat(pom, &estat) && estat.st_mtime != ns_cache_db_modtime)
      reopen = TRUE;
  }
  else
  {
  ...
  if(reopen)
  {
    ...
    ns_cache_db = dbopen(pom, O_BINARY | O_RDONLY, S_IRUSR | S_IWUSR, DB_HASH, &hash_info);
...
char *ns_cache_find_localname(char *urlstr)
{
  ...
  if(ns_cache_db)
  {
    ...
    {
      retv = tl_str_concat(DBGvars(NULL), cfg.ns_cache_dir, (tl_is_dirname(cfg.ns_cache_dir) ? "" : "/"),
