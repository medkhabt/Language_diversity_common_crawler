...
html_parser_t *html_parser_init(html_tag_t * tags, int ntags, int with_tag_rewriting, int purestyle, int purescript)
{
  html_parser_t *rv;
  int i;
  ...
  else
  {
    ...
    rv->tag_hash = dlhash_new(20, html_parser_tag_key_func, html_parser_tag_hash_func, html_parser_tag_comp_func);
...
#define KILL_FUNC_CHAIN(chain) \
  while (chain) \
  { \
    if(chain->data) _free(chain->data);\
...
void html_parser_add_tag_func(html_parser_t * hpinfo, html_parser_func_t func, void *data)
{
  html_parser_func_info_t *nfunc;

  nfunc = _malloc(sizeof(*nfunc));
  nfunc->func = func;
  nfunc->data = data;
  hpinfo->tag_funcs = dllist_append(hpinfo->tag_funcs, (dllist_t) nfunc);
}

void html_parser_add_attrib_func(html_parser_t * hpinfo, html_parser_func_t func, void *data)
{
  html_parser_func_info_t *nfunc;
...
void html_parser_add_style_func(html_parser_t * hpinfo, html_parser_func_t func, void *data)
{
  html_parser_func_info_t *nfunc;
...
void html_parser_add_script_func(html_parser_t * hpinfo, html_parser_func_t func, void *data)
{
  html_parser_func_info_t *nfunc;
...
void html_parser_set_document(html_parser_t * hpinfo, url * doc_url, char *content, ssize_t size)
{
...
void html_parser_take_document(html_parser_t * hpinfo, char **out_content, ssize_t * out_size)
{
...
static void html_parser_process_new_base_url(html_parser_t * hpinfo, char *baseattr)
{
  url *purl;
  char *newbase;
  ...
  else
  {
    ...
    newbase = url_to_absolute_url(hpinfo->base, hpinfo->baset, hpinfo->doc_url, baseattr);

    if(!newbase)
      return;                   /* collect base="" and ignore it */
...
static void html_parser_flush_stack_to_output(html_parser_t * hpinfo)
{
  ...
  html_parser_MEXPAND(hpinfo, l) memcpy(hpinfo->out_content + hpinfo->out_offset, hpinfo->stack, l);
  hpinfo->out_offset += l;
...
static int html_parser_check_tag(html_parser_t * hpinfo, char *tagstart)
{
  ...
  if(strchr(" \t\r\n>", tagstart[tl]))
  {
    ...
    hpinfo->current_tag = (html_tag_t *) dlhash_find_by_key(hpinfo->tag_hash, (dllist_t) tagname);
    _free(tagname);
  }
...
void html_parser_parse(html_parser_t * hpinfo)
{
  ...
  for(p = hpinfo->in_content; (p - hpinfo->in_content) < hpinfo->in_size; p++, hpinfo->in_offset++)
  {
    if(stylestart)
    {
      ...
      {
        html_parser_SEXPAND(hpinfo, 1) hpinfo->stack[hpinfo->stack_offset] = *p;
        hpinfo->stack_offset++;
      }
...
void html_parser_parse_tag(html_parser_t * hpinfo, char *stack, void *data)
{
  ...
  for(j = 0; hpinfo->current_tag->attribs[j].attrib; j++)
  {
    ...
    hpinfo->tag_attrib = html_get_attrib_from_tag(hpinfo->stack, hpinfo->current_attrib->attrib);
    ...
    for(ptr = cfg.dont_touch_url_rpattern; ptr && hpinfo->tag_attrib; ptr = ptr->next)
    {
    ...
    for(ptr = cfg.dont_touch_tag_rpattern; ptr && hpinfo->tag_attrib; ptr = ptr->next)
    {
    ...
    if(hpinfo->rewrite && hpinfo->tag_attrib)
    {
      ...
      html_replace_url_in_stack(hpinfo->stack, hpinfo->current_attrib->attrib, hpinfo->tag_attrib, FALSE);
    }
...
void html_parser_parse_tag_slash_a(html_parser_t * hpinfo, char *stack, html_extract_info_t * einfo)
{
...
void html_parser_parse_tag_meta_refresh(html_parser_t * hpinfo, char *stack, void *data)
{
  ...
  if(hpinfo->tag_attrib)
  {
    ...
    if(hpinfo->rewrite)
    {
      ...
      saved_meta = _realloc(saved_meta, strlen(saved_meta) + strlen(hpinfo->tag_attrib) + 4);
...
void html_parser_parse_tag_meta_robots(html_parser_t * hpinfo, char *stack, html_robots_info_t * oinfo)
{
  char *meta_type;
  char *content;
  char **flags;
...
void html_parser_parse_tag_jstransform(html_parser_t * hpinfo, char *stack, void *data)
{
#ifdef HAVE_REGEX
  dllist *ptr;
  ...
  for(ptr = priv_cfg.js_transform; ptr; ptr = ptr->next)
  {
    ...
    if(js_transform_match_tag(jt, hpinfo->stack))
    {
      ...
      if(hpinfo->rewrite && jt->type == 1 && nsub)
      {
        ...
        memmove(attr + l + subs[2], attr + subs[3], strlen(attr + subs[3]) + 1);
...
/********************************************************/
void html_parser_url_to_absolute_url(html_parser_t * hpinfo, char *stack, void *data)
{
  char *ustr;

  /*
     xprintf(0, "http_parser sees %s %s=\"%s\"<\n",
     hpinfo->current_tag->tag,
     hpinfo->current_attrib->attrib,
     hpinfo->tag_attrib);
   */

  ustr = url_to_absolute_url(hpinfo->base, hpinfo->baset, hpinfo->doc_url, hpinfo->tag_attrib);
...
{
  if(hpinfo->current_tag->type == HTML_TAG_BASE && hpinfo->current_attrib->type == HTML_ATTRIB_HREF)
  {
    int lp, ls;
...
void html_parser_process_form(html_parser_t * hpinfo, char *stack, dllist ** formlist)
{
  if(hpinfo->current_attrib->stat & LINK_FORM && hpinfo->current_attrib->type == HTML_ATTRIB_ACTION)
  {
    DEBUG_HTMLFORM
      ("html_parser_process_form FORM detected URL:'%s', has %s formlist\n", hpinfo->tag_attrib, (formlist != NULL ? "a" : "NO"));

    hpinfo->doc_url->status |= URL_HAVE_FORMS;

    if(formlist && hpinfo->tag_attrib)
    {
      *formlist = dllist_append(*formlist, (dllist_t) tl_strdup(hpinfo->tag_attrib));
    }
  }
}

void html_parser_get_url(html_parser_t * hpinfo, char *stack, html_extract_info_t * einfo)
{
#if 1                           /* [i_a] 2006-1-19 turn on */
  DEBUG_TRACE("html_parser_get_url URL:'%s'\n", hpinfo->tag_attrib);
  if(hpinfo->current_tag)
  {
    DEBUG_TRACE("html_parser_get_url hpinfo->current_tag->type bits:%04X\n", hpinfo->current_tag->type);
  }
  DEBUG_TRACE("html_parser_get_url hpinfo->current_attrib->type bits:%04X\n", hpinfo->current_attrib->type);
  DEBUG_TRACE("html_parser_get_url hpinfo->current_attrib->stat bits:%04X\n", hpinfo->current_attrib->stat);
  DEBUG_TRACE("html_parser_get_url hpinfo->current_attrib->stat & LINK_DOWNLD:%d\n", (hpinfo->current_attrib->stat & LINK_DOWNLD));
  DEBUG_TRACE("html_parser_get_url einfo->only_inline:%d\n", einfo->only_inline);
  DEBUG_TRACE("html_parser_get_url hpinfo->current_attrib->stat & LINK_INLINE:%d\n", (hpinfo->current_attrib->stat & LINK_INLINE));
  DEBUG_TRACE("html_parser_get_url hpinfo->current_attrib->stat & LINK_SCRIPT:%d\n", (hpinfo->current_attrib->stat & LINK_SCRIPT));
  DEBUG_TRACE("html_parser_get_url einfo->enable_js:%d\n", einfo->enable_js);
#endif
      ...
      (einfo->enable_js && (hpinfo->current_attrib->stat & LINK_SCRIPT))))
  {
    ...
    condp.attrib = hpinfo->current_attrib ? hpinfo->current_attrib->attrib : NULL;
    ...
    {
      /* [i_a] 2006 */
      char *urlstr = url_to_urlstr(purl, TRUE);
      DEBUG_HTML("URL parsed -> %s\n", urlstr);
      _free(urlstr);
    }
#endif
    url_path_abs(purl);
    ...
    if(hpinfo->current_attrib->stat & LINK_FORM)
      purl->status |= URL_FORM_ACTION;
#endif

    purl->level = hpinfo->doc_url->level + 1;
    purl->parent_url = dllist_append(purl->parent_url, (dllist_t) hpinfo->doc_url);
      ...
      html_tag_co_elem(hpinfo->stack, "ISMAP"))
    {
      ...
      _free(einfo->prev_a->data);
      einfo->urls = dllist_remove_entry(einfo->urls, einfo->prev_a);
      einfo->prev_a = NULL;
    }

    if(einfo->prev_a && hpinfo->current_tag->type == HTML_TAG_A && hpinfo->current_attrib->type == HTML_ATTRIB_HREF)
    {
#if 0
      free_deep_url((url *) einfo->prev_a->data);
      _free((url *) einfo->prev_a->data);
      einfo->urls = dllist_remove_entry(einfo->urls, einfo->prev_a);
#endif
...
#if 01
    DEBUG_TRACE("html_parser_get_url einfo->no_limits:%d\n", einfo->no_limits);
#endif
      ...
      hpinfo->tag_attrib[0] == '#')
    {
      ...
      free_deep_url(purl);      /* [i_a] memleak fix! */
      _free(purl);
    }
    else if(einfo->no_limits || url_append_condition(purl, &condp))
    {
      ...
      if(hpinfo->current_tag->type == HTML_TAG_A && hpinfo->current_attrib->type == HTML_ATTRIB_HREF)
      {
...
void html_parser_url_to_local(html_parser_t * hpinfo, char *stack, html_rewrite_info_t * rinfo)
{
  url *urlp, *before_url;
  char *anchor, *fn;
  int is_local;
  ...
  if(!before_url)
  {
    ...
    ptr = dllist_find2(rinfo->einfo->urls, (dllist_t) urlp, dllist_url_compare);
    if(ptr)
  ...
  is_local = !tl_access(fn, R_OK);

  if(is_local || rinfo->all_to_local || (rinfo->selected_to_local && before_url) || url_compare(urlp, hpinfo->doc_url))
  {
    char *actname, *relname;
    struct stat estat;

    if(is_local && !tl_stat(fn, &estat) && S_ISDIR(estat.st_mode))
      fn = tl_str_concat(DBGvars(NULL), fn, (tl_is_dirname(fn) ? "" : "/"), priv_cfg.index_name, NULL);
    else
      fn = tl_strdup(fn);
    ...
    if(anchor)
      relname = tl_str_concat(DBGvars(relname), "#", anchor, NULL);
...
void html_parser_remove_advertisement(html_parser_t * hpinfo, char *stack, void *data)
{
#ifdef HAVE_REGEX
  int is_adver = FALSE;

  if(hpinfo->current_tag->type != HTML_TAG_IMG || hpinfo->current_attrib->type != HTML_ATTRIB_SRC)
    return;
...
void html_parser_change_url(html_parser_t * hpinfo, char *stack, html_change_info_t * chinfo)
{
  url *urlp;
...
/********************************************************/
void html_parser_style_to_absolute_urls(html_parser_t * hpinfo, char *stack, void *data)
{
  char *alttag;
  ...
  if(hpinfo->tag_attrib)
  {
    alttag = css_to_absolute_links(hpinfo->doc_url, hpinfo->tag_attrib, hpinfo->base, hpinfo->baset);
    _free(hpinfo->tag_attrib);
  ...
  else
  {
    ...
    alttag = css_to_absolute_links(hpinfo->doc_url, hpinfo->stack, hpinfo->base, hpinfo->baset);
...
void html_parser_get_style_urls(html_parser_t * hpinfo, char *stack, html_extract_info_t * einfo)
{
  dllist *pv;
  if(!cfg.read_css)
  ...
  if(hpinfo->tag_attrib)
  {
    pv = css_get_all_links(hpinfo->doc_url, hpinfo->tag_attrib, hpinfo->base, hpinfo->baset, einfo->no_limits);
  }
  else
  {
    pv = css_get_all_links(hpinfo->doc_url, hpinfo->stack, hpinfo->base, hpinfo->baset, einfo->no_limits);
  }

  einfo->urls = dllist_concat(einfo->urls, pv);
}

void html_parser_style_to_local_urls(html_parser_t * hpinfo, char *stack, html_rewrite_info_t * rinfo)
{
  char *alttag;
  ...
  if(hpinfo->tag_attrib)
  {
    alttag = css_remote_to_local_links(hpinfo->doc_url,
      hpinfo->tag_attrib, rinfo->all_to_local, rinfo->selected_to_local, hpinfo->base, hpinfo->baset);
    _free(hpinfo->tag_attrib);
    hpinfo->tag_attrib = alttag;
  }
  else
  {
    ...
    alttag = css_remote_to_local_links(hpinfo->doc_url,
      hpinfo->stack, rinfo->all_to_local, rinfo->selected_to_local, hpinfo->base, hpinfo->baset);
...
void html_parser_style_change_url(html_parser_t * hpinfo, char *stack, html_change_info_t * chinfo)
{
  char *alttag;
  ...
  if(hpinfo->tag_attrib)
  {
    alttag = css_change_url(hpinfo->doc_url, hpinfo->tag_attrib, chinfo->url_old, chinfo->url_new);
    _free(hpinfo->tag_attrib);
    hpinfo->tag_attrib = alttag;
  }
  else
  {
    ...
    alttag = css_change_url(hpinfo->doc_url, hpinfo->stack, chinfo->url_old, chinfo->url_new);
...
/***********************************************************/
void html_parser_parse_jspatterns(html_parser_t * hpinfo, char *stack, void *data)
{
#ifdef HAVE_REGEX
  dllist *ptr;
  int found = FALSE;
  ...
  {
    if(re_pmatch_sub((re_entry *) ptr->data, hpinfo->tag_attrib, 1, &start, &end))
    {
      found = TRUE;
      break;
    }
  ...
  if(found && (start >= 0))
  {
    ...
    new_attrib = _malloc(start + strlen(saved_attrib + end) + strlen(hpinfo->tag_attrib) + 1);
...
void html_parser_parse_body_jspatterns(html_parser_t * hpinfo, char *stack, void *data)
{
#ifdef HAVE_REGEX
  char *stackc = NULL;
  char *p;
  ...
  while(*p)
  {
    ...
    if(hpinfo->rewrite)
      stackc = tl_str_concat(DBGvars(stackc), hpinfo->tag_attrib, "\n", NULL);
    _free(hpinfo->tag_attrib);
...
void html_parser_parse_body_jstransform(html_parser_t * hpinfo, char *stack, void *data)
{
#ifdef HAVE_REGEX
  char *p;
  int ilen;
  ...
  while(*p)
  {
    ...
    if(hpinfo->rewrite)
      stackc = tl_str_concat(DBGvars(stackc), ln, "\n", NULL);
