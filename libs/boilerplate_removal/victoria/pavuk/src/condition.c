...
static int cond_nohttp(url *, cond_info_t *);
...
#define CL0 (1 << 0) /* links decoding : css_get_all_links(), html_parser_get_url() */
#define CL1 (1 << 1) /* links/URLs collection : cat_links_to_url_list() */
#define CL2 (1 << 2) /* document processing : process_document() */
#define CL3 (1 << 3) /* FTP/HTTP: responses : ftp_connect(), http_process_response() */
...
static const struct cond_type_info_t cond_type_info[] =
{
  ...
  { CONDT_NOHTTP, "-noHTTP", cond_nohttp, TRUE, CL0 | CL2 },
  ...
  { CONDT_DONT_LEAVE_SITE, "-dont_leave_site", cond_dont_leave_site, TRUE, CL0 | CL2 },
  { CONDT_DONT_LEAVE_DIR, "-dont_leave_dir", cond_dont_leave_dir, TRUE, CL0 | CL2 },
  { CONDT_SITE_LEVEL, "-site_level", cond_site_level, TRUE, CL0 | CL2 },
  { CONDT_LEAVE_LEVEL, "-leave_level", cond_leave_level, TRUE, CL0 | CL2 },
  { CONDT_DONT_LEAVE_SITE_ENTER_DIR, "-dont_leave_site_enter_dir", cond_dont_leave_site_enter_dir, TRUE, CL0 | CL2 },
  ...
  { CONDT_SKIP_RPATTERN, "-skip_rpattern", cond_skip_rpattern, FALSE, CL0 | CL2 },
  ...
  { CONDT_SKIP_URL_PATTERN, "-skip_url_pattern", cond_skip_url_pattern, FALSE, CL0 | CL2 },
  { CONDT_SKIP_URL_RPATTERN, "-skip_url_rpattern", cond_skip_url_rpattern, FALSE, CL0 | CL2 },
  { CONDT_TAG_PATTERN, "-tag_pattern", cond_tag_pattern, TRUE, CL0 },
  { CONDT_TAG_RPATTERN, "-tag_rpattern", cond_tag_rpattern, TRUE, CL0 },
  { CONDT_USER_CONDITION, "-user_condition", cond_user_condition, FALSE, CL1 | CL3 },
};
...
#ifdef HAVE_REGEX
static int url_append_condition_patterns_default(url *urlp, cond_info_t *condp)
{
  int pm1, pm2, pm3, pm4;
  ...
  if (priv_cfg.condition.skip_url_pattern && priv_cfg.condition.rskip_url_pattern)
  {
    if (!pm2 && !pm4)
    {
      if (!pm2)
...
static int url_append_condition_default(url *urlp, cond_info_t *condp)
{
  ...
  {
    /* [i_a] 2006 */
    char *urlstr = url_to_urlstr(urlp, TRUE);
    DEBUG_DEVEL("url_append_condition_default: URL: %s\n", urlstr);
    _free(urlstr);
  }
#endif

  level = (1 << condp->level);
  DEBUG_DEVEL("url_append_condition_default: level = %d\n", level);
  DEBUG_DEVEL("url_append_condition_default: cfg.condition.limit_inlines = %d\n",
              cfg.condition.limit_inlines);
  DEBUG_DEVEL("url_append_condition_default: (urlp->status & URL_INLINE_OBJ) = %d\n",
              (urlp->status & URL_INLINE_OBJ));

  if (!cfg.condition.limit_inlines && (urlp->status & URL_INLINE_OBJ))
  {
    DEBUG_DEVEL("url_append_condition_default: TRUE (A)\n");
    return TRUE;
  }
  ...
  for (i = 0; i < NUM_ELEM(cond_type_info); i++)
  {
    ...
    {
      DEBUG_DEVEL("url_append_condition_default: cond_type_info[i = %d]\n", i);
      DLMSG(i);
      return FALSE;
    }
  }
  ...
  DEBUG_DEVEL
  ("url_append_condition_default: cond_type_info[CONDT_PATTERN].level = %d -- & level: %d\n",
   cond_type_info[CONDT_PATTERN].level,
   (cond_type_info[CONDT_PATTERN].level & level));
  if ((cond_type_info[CONDT_PATTERN].level & level))
  {
    if (!url_append_condition_patterns_default(urlp, condp))
    {
      DEBUG_DEVEL("url_append_condition_default: url_append_condition_patterns_default = FALSE\n");
      return FALSE;
    }
  }

  DEBUG_DEVEL(
    "url_append_condition_default: cond_type_info[CONDT_USER_CONDITION].level = %d -- & level: %d\n",
    cond_type_info[CONDT_USER_CONDITION].level,
    (cond_type_info[CONDT_USER_CONDITION].level & level));
  if ((cond_type_info[CONDT_USER_CONDITION].level & level))
  {
    if (!cond_user_condition(urlp, condp))
    {
      DEBUG_DEVEL("url_append_condition_default: cond_user_condition = FALSE\n");
      DLMSG(CONDT_USER_CONDITION);
      return FALSE;
    }
  }

  DEBUG_DEVEL("url_append_condition_default: TRUE (B)\n");
  return TRUE;
}

/********************************************************/
/* check whether URL match given limiting conditions */
/********************************************************/
...
/********************************************************/
/* check whether site is from one of domains from array */
/********************************************************/
static bool_t domain_condition(const char *site, const char **l)
{
  const char **p = l;
  int sl = strlen(site);
...
/********************************************************/
/* check whether up have suffix sfx      */
/********************************************************/
static bool_t cmp_sfx(const char *up, const char *sfx)
{
  const char *pom = NULL;
  int nlen, slen;
  int rv;
...
/********************************************************/
/* check whether url have match one of suffixes from array */
/********************************************************/
static bool_t sfx_condition(url *urlr, const char **l)
{
  const char **pp = l;
  const char *p = url_get_full_path(urlr);
  int rv = FALSE;
...
/********************************************************/
/* check whether url path have one of prefixes from  */
/* priv_cfg.condition.dir_prefix      */
/********************************************************/
static bool_t prefix_condition(url *urlr, const char **l)
{
  const char **pp = l;
  const char *p = url_get_full_path(urlr);
  int rv = FALSE;
...
/********************************************************/
/* check whether string is matched by at least one */
/* wildcard pattern from list       */
/********************************************************/
static bool_t cmp_pattern(char *str, const char **pattern)
{
  const char **pp;
...
static bool_t cmp_dlpattern(const char *str, dllist *pattern)
{
  for ( ; pattern; pattern = pattern->next)
  {
    if (!fnmatch((const char *)pattern->data, str, 0))
      return FALSE;
  }
...
static const char **str_list_to_array(dllist *sl)
{
  const char **rv;
  int i;

  rv = _malloc(sizeof(rv[0]) * (dllist_count(sl) + 1));

  for (i = 0; sl; i++, sl = sl->next)
    rv[i] = (const char *)sl->data;
  rv[i] = NULL;
...
/********************************************************/
static bool_t cmp_rpattern(const char *str, dllist *pattern)
{
  dllist *pp;
...
static dllist *str_list_to_re_list(dllist *sl)
{
  ...
  for ( ; sl; sl = sl->next)
  {
    re_entry *re = re_make((const char *)sl->data);
...
/********************************************************/
/* check whether site of URL matches one of listed IP  */
/* address regular patterns        */
/*              */
/* TRUE  = match          */
/* FALSE = don't match          */
...
static int cond_unsupported(url *urlp, cond_info_t *condp)
{
   ...
   * < - sound   x
   */
  if ((urlp->type == URLT_GOPHER) && strchr("0124569gIhs:;<", urlp->p.gopher.selector[0]))
  {
    return TRUE;
  }
...
#if 0
  xprintf(1,
          "cond_unsupported: prottable[type = %d] = %s\n",
          urlp->type,
          prottable[urlp->type].urlid);
  DLMSG(CONDT_UNSUP);
#endif
  return FALSE;
}
...
static int cond_lmax(url *urlp, cond_info_t *condp)
{
  ...
  {
    return (urlp->level - ((urlp->status & URL_INLINE_OBJ) ? 1 : 0)) <= cfg.condition.max_levels;
  }
  else
  {
    return TRUE;
...
static int cond_dmax(url *urlp, cond_info_t *condp)
{
  ...
  if (n)
  {
    return !((!condp->urlnr && (cfg.total_cnt + 1) > n) || (condp->urlnr && condp->urlnr > n));
  }
  else
  {
    return TRUE;
...
static int cond_nohttp(url *urlp, cond_info_t *condp)
{
...
{
  if ((urlp->type == URLT_HTTP || urlp->type == URLT_HTTPS) && !cfg.condition.cgi)
  {
...
static int cond_asite(url *urlp, cond_info_t *condp)
{
  ...
  else if (priv_cfg.condition.sites && priv_cfg.condition.sites[0] && site &&
...
static int cond_adomain(url *urlp, cond_info_t *condp)
{
  ...
  if (condp->params)
  {
    const char **sa = str_list_to_array(condp->params);
    int rv = domain_condition(site, sa);
    _free(sa);
    return rv;
  }
  else if (priv_cfg.condition.domains && priv_cfg.condition.domains[0] && site &&
           priv_cfg.condition.allow_domain)
  {
...
static int cond_aport(url *urlp, cond_info_t *condp)
{
  ...
  {
    return dllist_find(priv_cfg.condition.ports, (dllist_t)port) ? TRUE : FALSE;
  }
  else
  {
    return TRUE;
...
static int cond_amime_type(url *urlp, cond_info_t *condp)
{
  ...
  else if (priv_cfg.condition.mime && priv_cfg.condition.allow_mime && condp->mimet)
  {
