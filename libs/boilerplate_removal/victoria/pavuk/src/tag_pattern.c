...
tag_pattern_t *tag_pattern_new(int type, char *tag, char *attrib, char *urlp)
{
  ...
  if(type == TAGP_WC)
  {
    ...
    {
      xvperror(DBGvars(NULL), gettext("Bad -tag_pattern tag wildcard pattern: '%s'\n"), tag);
    }

    r[1] = fnmatch(attrib, "", 0);
    if(r[1] != 0 && r[1] != FNM_NOMATCH)
    {
      xvperror(DBGvars(NULL), gettext("Bad -tag_pattern attribute wildcard pattern: '%s'\n"), rv->attrib);
    }

    r[2] = fnmatch(urlp, "", 0);
    if(r[2] != 0 && r[2] != FNM_NOMATCH)
    {
      xvperror(DBGvars(NULL), gettext("Bad -tag_pattern url wildcard pattern: '%s'\n"), rv->attrib);
    }
...
int tag_pattern_match(tag_pattern_t * tp, char *tag, char *attrib, char *urlp)
{
  if(tp->type == TAGP_WC)
  {
    return (!fnmatch(tp->tag, tag, 0) && !fnmatch(tp->attrib, attrib, 0) && !fnmatch(tp->urlp, urlp, 0));
  }
#ifdef HAVE_REGEX
  else if(tp->type == TAGP_RE)
  {
    return (re_pmatch(tp->tag_re, tag) && re_pmatch(tp->attrib_re, attrib) && re_pmatch(tp->urlp_re, urlp));
  }
