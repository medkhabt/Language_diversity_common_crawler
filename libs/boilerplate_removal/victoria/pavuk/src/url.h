...
/*
   WARNING: prottable[] in url.c expects this particular order and implicit
...
#define URL_ISHAMMER            (unsigned int) (1 << 21)
#endif
...
typedef struct
{
  ...
  dllist *fields;
  cookie_entry *cookies;        /* add these cookies when you hit this url */
  char *localname;
} url_info;

extern url_info *url_info_new(char *);
extern url_info *url_info_parse(const char *str, bool_t try_me);
extern char *url_info_dump(url_info *);
extern void url_info_free(url_info *);
extern url_info *url_info_duplicate(url_info *);

extern url *url_parse(DBGdecl(char *));
#define url_parse(u)    url_parse(DBGvars(u))
extern url *url_dup_url(DBGdecl(url *));
#define url_dup_url(u)  url_dup_url(DBGvars(u))
...
extern url *new_url(DBGdecl(url *));
...
extern char *url_to_urlstr(DBGdecl(url * urlp), int with_anchor);
#define url_to_urlstr(u, n)       url_to_urlstr(DBGvars(u), n)
extern char *url_to_request_urlstr(DBGdecl(url *), int);
#define url_to_request_urlstr(u, l)             url_to_request_urlstr(DBGvars(u), l)
extern void url_path_abs(DBGdecl(url *));
#define url_path_abs(u)         url_path_abs(DBGvars(u))
extern url *filename_to_url(char *);
extern void cat_links_to_url_list(dllist *);
extern void append_url_to_list(url *);
extern void append_url_list_to_list(dllist *, dllist * after);
