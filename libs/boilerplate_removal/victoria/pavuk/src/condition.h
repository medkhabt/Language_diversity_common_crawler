...
typedef struct
{
  ...
  long leave_level;             /*** how many levels leave from starting site ***/
  ...
  long max_size;                /*** maximal size of document to exclude big documents ***/
  long min_size;                /*** minimal size of document to exclude small documents ***/
  long max_levels;              /*** obmedzenie na maximalnu hlbku vnorenia sa do stromu - 0 neobmedzene ****/
  long max_documents;           /*** obmedzenie na maximalny pocet dokumentov - 0 neobmedzene ***/
  bool_t cgi;                   /*** povolenie / zakazanie prenosu dokumentov enerovanych cez CGI ***/
  const char **sites;           /*** zoznam povolenych / zakazanych serverov ***/
  bool_t allow_site;            /*** indikacia ci v "sites" su povolene alebo zakazane servery ***/
  const char **sufix;           /*** zoznam povolenych / zakazanych pripon dokumentov ***/
  bool_t allow_sufix;           /*** indikacia ci v zozname "sufix" su povolene alebo zakazane pripony ***/
  const char **dir_prefix;      /*** zozname povolenych / zakazanych prefixov dokumentov ***/
  bool_t allow_prefix;          /*** indikacia ci v zozname "dir_prefix" su povolene alebo zakazane prefixy ***/
  const char **domains;         /*** list of domains to check in URLs ***/
  bool_t allow_domain;          /*** domains in list are allowed ***/
  const char **mime;            /*** list of mime types to check ***/
  ...
  const char **pattern;               /*** fnmatch patterns for matching document names ***/
  const char **url_pattern;           /*** fnmatch patterns for matching url ***/
  const char **skip_pattern;          /*** fnmatch patterns to exclude matching document names ***/
  const char **skip_url_pattern;      /*** fnmatch patterns to exclude matching url ***/
  char *uexit;                  /*** user exit script ***/
  char *follow_cmd;             /*** user exit script to allow/disallow following links of current doc ***/
  long site_level;              /*** maximum site level to leave ***/
...
/*
   WARNING: cond_type_info[] in condition.c expects this particular order and implicit
   numbering as several locations in the code use this enum as a direct index
   into that table!
 */
typedef enum
{
