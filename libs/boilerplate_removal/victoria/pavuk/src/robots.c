...
#ifdef HAVE_MT
#define LOCK_ROBOTS_ENTRY(ent)  mt_pthread_mutex_lock(&(ent)->lock, "robots entry", __LINE__, __FILE__)
#define UNLOCK_ROBOTS_ENTRY(ent) mt_pthread_mutex_unlock(&(ent)->lock, "robots entry", __LINE__, __FILE__)
#else
...
bool_t may_do_a_robots_check(url *urlp)
{
  if ((urlp->type != URLT_HTTP && urlp->type != URLT_HTTPS) || !cfg.condition.allow_robots)
    return FALSE;
...
bool_t robots_check(url *urlp)
{
  ...
  {
    while (robots[i]
           && !(!strcmp(robots[i]->site, urlp->p.http.host)
  ...
  if (dont_have)
  {
    tmpr = (robotlim *)_malloc(sizeof(*tmpr));
...
//    docu.actrec_ref = NULL;
...
#if defined (HAVE_DEBUG_FEATURES)
  {
    ...
    DEBUG_PROTOD("********************* Content: ROBOTS.TXT **************\n");
  ...
  else if (docu.errcode == ERR_HTTP_NFOUND || docu.errcode == ERR_HTTP_GONE)
  {
    ...
    if ((f = tl_open(pp, O_BINARY | O_CREAT | O_TRUNC | O_WRONLY,
