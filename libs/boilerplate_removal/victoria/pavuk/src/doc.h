...
typedef struct
{
  ...
  char *request_header;          /*** transmitted request header ***/
  char *request_data;            /*** transmitted request data ***/
  ssize_t request_datalen;       /*** size of transmitted request data ***/
  ...
  activity_record *actrec_ref;   /*** temporary reference to the related activity store record ***/
  int cyclenr;                   /*** hammering only: current replay cycle ***/
#endif
  ...
  bool_t ftp_dir_type;           /*** This is a (1) LIST or (0) NLST-based directory listing ***/

  /*******************************/
  /* progress meter informations */
  /*******************************/
  struct timeval start_time;     /*** for progress metter ***/
  struct timeval hr_start_time;  /*** when document processing started ***/
  ...
  /* [i_a] removed GETTIMEOFDAY dependency! */
