...
void mopt_init(mopt_t * mopt, int nparams, cfg_param_t * params, int argc, char **argv)
{
  int s_size, l_size;
  int i;
  ...
  mopt->short_hash = dlhash_new(s_size, mopt_short_key_func, mopt_short_hash_func, mopt_short_comp_func);

  mopt->long_hash = dlhash_new(l_size, mopt_long_key_func, mopt_long_hash_func, mopt_long_comp_func);
...
/*
   ...
   on unknown param          - -2
   on wrong formatted param  - -3
 */
int mopt_get_next_param(mopt_t * mopt, cfg_param_t ** pparam)
{
  cfg_param_t *param = *pparam = NULL;  /* [i_a] */
  char *cpar;
  int i, np;
  ...
  if(mopt->short_offset)
  {
    ...
    if(mopt->short_offset == 1)
      param = (cfg_param_t *) dlhash_find_by_key(mopt->short_hash, (dllist_t) cpar);
    ...
    else
    {
      ...
      param = (cfg_param_t *) dlhash_find_by_key(mopt->short_hash, (dllist_t) pom);
    ...
    np = cfg_get_num_params(param, mopt->option_type);
  ...
  {
    param = (cfg_param_t *) dlhash_find_by_key(mopt->long_hash, (dllist_t) (cpar + ((cpar[0] == '-' && cpar[1] == '-') ? 1 : 0)));
    ...
    else if(cpar[0] == '-' && cpar[1] == '-' && strchr(cpar, '='))
    {
      ...
      param = (cfg_param_t *) dlhash_find_by_key(mopt->long_hash, (dllist_t) (tpar + 1));
