...
char *gprop_dump(gprop * gp)
{
  ...
  *pom = 0;
  ...
  {
  case GPROP_STR:
    snprintf(pom, sizeof(pom), "%s%s=%s", typetab[gp->type].id, gp->name, (char *) gp->value);
    pom[sizeof(pom) - 1] = 0;
    break;
  case GPROP_INT:
    ...
    break;
  case GPROP_BOOL:
    snprintf(pom, sizeof(pom), "%s%s=%s", typetab[gp->type].id, gp->name, gp->value ? "true" : "false");
    pom[sizeof(pom) - 1] = 0;
    break;
  }
