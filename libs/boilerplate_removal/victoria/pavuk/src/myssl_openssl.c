...
static SSL_SESSION *cache_locate_session_by_addr(struct sockaddr_in *addr, int dolock)
{
  int i;
  ...
  for(i = 0; i < N_SESSIONS; i++)
  {
    if(session_map[i] && !cmp_sockaddr(addr, &session_map[i]->addr))
    {
      ...
      DEBUG_SSL("returning slot %d id for %s:%d\n", i, inet_ntoa(session_map[i]->addr.sin_addr), ntohs(session_map[i]->addr.sin_port));
...
static SSL_SESSION *ssl_get_session_cb(SSL * ssl, unsigned char *data, int len, int *copy)
{
...
/* SSL Certification stuff */
  if(priv_cfg.ssl_cert_file != NULL)
  {
...
#ifdef OPENSSL
      SSL_CTX_set_default_passwd_cb(con->ssl_ctx, (pem_password_cb *) my_ssl_passwd_callback);
#else
      SSL_CTX_set_default_passwd_cb(con->ssl_ctx, my_ssl_passwd_callback);
#endif

    if(SSL_CTX_use_certificate_file(con->ssl_ctx, priv_cfg.ssl_cert_file, SSL_FILETYPE_PEM) <= 0)
    {
    ...
    if(SSL_CTX_use_PrivateKey_file(con->ssl_ctx, priv_cfg.ssl_key_file, SSL_FILETYPE_PEM) <= 0)
    {
...
/* End SSL Certification stuff */
  ...
  while((rv = SSL_in_init(con->ssl_con)) && SSL_get_error(con->ssl_con, rv) == SSL_ERROR_WANT_CONNECT)
  {
    tl_msleep(2, FALSE);
  }
