...
#define TEXPAND(sv) \
  if((tsz - (sv + tr + BUFFER)) < 0) \
  { \
    text = _realloc(text, tsz + sv  + BUFFER); \
    tsz += BUFFER + sv; \
  }
  ...
  for(i = 0; i < html_len; i++)
  {
    ...
    if(html_text[i] == '<')
    {
      if(!strncasecmp(html_text + i + 1, "SCRIPT", 6) &&
        (i + 7) < html_len && (tl_ascii_isspace(html_text[i + 7]) || html_text[i + 7] == '>'))
      {
        scriptstart = TRUE;
        continue;
      }
...
int form_field_compare(dllist_t key1, dllist_t key2)
{
  form_field *ff1 = (form_field *) key1;
  form_field *ff2 = (form_field *) key2;
...
int form_field_compare_name(dllist_t key1, dllist_t key2)
{
  form_field *ff1 = (form_field *) key1;
  form_field *ff2 = (form_field *) key2;
...
{
  form_field_info *retv = _malloc(sizeof(*retv));
...
static form_info *form_parse_formtag(char *tag, url_info * ui)
{
  char *p;
  form_info *retv = _malloc(sizeof(*retv));

  DEBUG_HTMLFORM("form_parse_formtag: '%s'\n", tag);
#ifdef HAVE_DEBUG_FEATURES
  if (cfg.verbose)
  {
    char *dbgmsg = cvt_url_info2diag_str(ui);
    DEBUG_HTMLFORM("form_parse_formtag: URL INFO = %s", dbgmsg);
    _free(dbgmsg);
  }
#endif
  ...
  {
    DEBUG_HTMLFORM("form_parse_formtag detect method: '%s'\n", p);
    if(!strcasecmp(p, "GET"))
      retv->method = FORM_M_GET;
    ...
    {
      DEBUG_HTMLFORM("FORM_M_UNKNOWN: form_parse_formtag detected unidentified form method: '%s'\n", p);
  ...
  if((p = html_get_attrib_from_tag(tag, "enctype")))
  {
    ...
    else
    {
      ...
      DEBUG_HTMLFORM("FORM_E_UNKNOWN: form_parse_formtag detected unidentified form encoding: '%s'\n", p);
  ...
  /*
     [i_a] hack to make sure we don't get a 'filenamed' (localized and 'rewritten')
     URL for our ACTION.

     Case: Since we are processing a custom site which has an empty form with a
     <FORM> ACTION attribute written like 'page?query_args&', that action URL has
     already been munched by the rewrite logic in html_parse(), ending up as
     'page_query_args&' (note the underscore) in doc->contents by the time we
     get here.
     The easy way out would have been to check for the underscore, but that will
     not work as the page name itself also may have underscores. So we are left to
     'referring' the ACTION to the value that was decoded BEFORE all the html_parse()
     local rewrite logic was applied: THAT url (though expanded to a 'full' url
     like 'http://site:port/path/page?query_args&') can be found in the formdata
     list entry (url_info struct) for this form. Which is now passed along by
     add_matching_form() to form_parse() all the way up here...
   */
  if(ui)
  {
    if(ui->urlstr)
    {
      DEBUG_HTMLFORM("form_parse_formtag: ACTION overruled by UI -> ACTION: '%s' --> '%s'\n", retv->action, ui->urlstr);
      _free(retv->action);
      retv->action = tl_strdup(ui->urlstr);
    }
  }
...
form_info *form_info_dup(form_info * formi)
{
  ...
  {
    retv->infos = dllist_append(retv->infos, (dllist_t) form_field_duplicate((form_field *) ptr->data));
  }

  return retv;
}

form_info *form_parse(char *form_text, int form_len, url_info * ui)
{
  char *stack;
  int i, sr, ssz;
  int tagstart = FALSE;
...
#define IS_TAG(s) (!strncasecmp(stack, s, strlen(s)) && \
      ...
      if(IS_TAG("FORM"))
      {
        retv = form_parse_formtag(stack, ui);
        DEBUG_HTMLFORM("form_parse: FORM action = '%s'\n", (retv ? retv->action : "???"));
      }
      ...
      else if(selectgroup && IS_TAG("OPTGROUP"))
      {
        ...
        selectgroup->infos = dllist_append(selectgroup->infos, (dllist_t) lastformtag);
      }
      ...
      else if(selectgroup && IS_TAG("OPTION"))
      {
        ...
        if(optgroup)
        {
          optgroup->infos = dllist_append(optgroup->infos, (dllist_t) lastformtag);
        }
        else
        {
          selectgroup->infos = dllist_append(selectgroup->infos, (dllist_t) lastformtag);
      ...
      if(fstag && tr)
      {
        ...
        if(*sp)
        {
          ...
          if(optgroup)
          {
            ...
            optgroup->infos = dllist_insert_before(optgroup->infos, last, (dllist_t) ffi);
          }
          else if(selectgroup)
          {
            last = dllist_last(selectgroup->infos);
            selectgroup->infos = dllist_insert_before(selectgroup->infos, last, (dllist_t) ffi);
          }
          else
          {
            last = dllist_last(retv->infos);
            retv->infos = dllist_insert_before(retv->infos, last, (dllist_t) ffi);
          }
        }
        tr = 0;
      }
...
#else
        len = gtk_text_buffer_get_char_count(gtk_text_view_get_buffer(GTK_TEXT_VIEW(ffi->widget)));
#endif
...
static void form_submit_cb(GtkWidget * w, form_info * fi)
{
  ...
  if(ffi && ffi->name)
  {
    ...
    ff->value = ffi->default_value ? tl_strdup(ffi->default_value) : tl_strdup("");
...
static void form_reset(dllist * fields)
{
  while(fields)
  {
    ...
    case FORM_T_RADIO:
    case FORM_T_CHECKBOX:
      gtk_toggle_button_set_state(GTK_TOGGLE_BUTTON(ffi->widget), ffi->checked);
      break;
    case FORM_T_OPTGROUP:
    case FORM_T_SELECT:
      ...
      break;
    case FORM_T_OPTION:
      gtk_check_menu_item_set_state(GTK_CHECK_MENU_ITEM(ffi->widget), ffi->selected);
      break;
    case FORM_T_TEXTAREA:
#if GTK_FACE < 2
      gtk_text_set_point(GTK_TEXT(ffi->widget), 0);
      gtk_text_forward_delete(GTK_TEXT(ffi->widget), gtk_text_get_length(GTK_TEXT(ffi->widget)));
      if(ffi->default_value)
        gtk_text_insert(GTK_TEXT(ffi->widget), NULL, NULL, NULL, ffi->default_value, strlen(ffi->default_value));
#else
      gtk_text_buffer_set_text(gtk_text_view_get_buffer(GTK_TEXT_VIEW(ffi->widget)), ffi->default_value ? ffi->default_value : "", -1);
#endif
      break;
    default:
      break;
...
static GtkWidget *form_render_optgroup(form_field_info * sel, dllist * dl)
{
  ...
  while(ptr)
  {
    ...
    {
    case FORM_T_OPTION:
      if(ptr->next && (((form_field_info *) ptr->next->data)->type == FORM_T_NONFORM))
      {
        if(ffi->text)
          p = ffi->text;
        else
    ...
    case FORM_T_OPTGROUP:
      ...
      gtk_menu_item_set_submenu(GTK_MENU_ITEM(mi), form_render_optgroup(sel, ffi->infos));
      break;
    default:
...
static GtkWidget *form_render(form_info * fi, GtkWidget * pbox)
{
  ...
  for(ptr = fi->infos; ptr; ptr = ptr->next)
  {
    ...
    case FORM_T_TEXT:
      ...
      if(ffi->width)
        gtk_widget_set_usize(w, ffi->width * 10 /*gdk_string_width(w->style->font, "xxx") */ , -1);
      break;
    case FORM_T_PASSWORD:
      w = gtk_entry_new();
      if(ffi->maxlen)
    ...
    case FORM_T_CHECKBOX:
      if(ffi->text)
        p = ffi->text;
      else if(ptr->next && (((form_field_info *) ptr->next->data)->type == FORM_T_NONFORM))
      {
        p = ((form_field_info *) ptr->next->data)->text;
        ptr = ptr->next;
      }
    ...
    case FORM_T_RADIO:
      {
        ...
        while(ptr2)
        {
          ...
          if(ffi->type == tffi->type && tffi->name && ffi->name && !strcmp(tffi->name, ffi->name))
          {
    ...
    case FORM_T_IMAGE:
      ...
      gtk_signal_connect(GTK_OBJECT(w), "clicked", GTK_SIGNAL_FUNC(form_submit_cb), fi);
      break;
    case FORM_T_RESET:
      if(ffi->name)
        p = ffi->name;
      ...
      gtk_signal_connect(GTK_OBJECT(w), "clicked", GTK_SIGNAL_FUNC(form_reset_cb), fi);
      break;
    case FORM_T_FILE:
      w = gtk_entry_new();
      break;
...
static void form_edit_dlg_refresh_list(GtkWidget * w, struct form_edit_dlg_t *fedlg)
{
  int i;
...
static void form_edit_dlg_browse(struct form_edit_dlg_t *fedlg)
{
  ...
  gtk_clist_set_selection_mode(GTK_CLIST(fedlg->url_list), GTK_SELECTION_BROWSE);
  gtk_clist_set_column_title(GTK_CLIST(fedlg->url_list), 0, gettext("URLs with forms"));
  ...
  button = guitl_pixmap_button(restart_small_xpm, NULL, gettext("Refresh URL list"));
  gtk_box_pack_start(GTK_BOX(vbox), button, FALSE, FALSE, 10);
  gtk_widget_show(button);

  gtk_signal_connect(GTK_OBJECT(button), "clicked", GTK_SIGNAL_FUNC(form_edit_dlg_refresh_list), fedlg);
}

static void form_edit_dlg_switch_form(GtkWidget * w, struct form_edit_dlg_t *fedlg)
{
  char *formdata;
  int formlen;
  ...
  fedlg->formdata = form_parse(formdata, formlen, NULL);
  fedlg->formdata->parent_url = fedlg->parent_url;

  gtk_label_set(GTK_LABEL(fedlg->method),
    (fedlg->formdata->method == FORM_M_GET) ? "GET" : (fedlg->formdata->method == FORM_M_POST) ? "POST" : gettext("unknown"));
  gtk_label_set(GTK_LABEL(fedlg->encoding),
    (fedlg->formdata->encoding ==
      FORM_E_URLENCODED) ? "application/x-www-form-urlencoded" : (fedlg->
      formdata->encoding == FORM_E_MULTIPART) ? "multipart/form-data" : gettext("unknown"));
  gtk_label_set(GTK_LABEL(fedlg->action), fedlg->formdata->action);

  fedlg->form = gtk_vbox_new(FALSE, 2);
  gtk_scrolled_window_add_with_viewport(GTK_SCROLLED_WINDOW(fedlg->swin), fedlg->form);
...
static char *form_load_doc(char *docfile, int *doclen)
{
  ...
  *doclen = 0;
  ...
  if((fd = tl_open(docfile, O_BINARY | O_RDONLY, S_IRUSR | S_IRGRP | S_IROTH | S_IWUSR)) < 0)
  {
    xperror(mk_native(docfile));
    return NULL;
  }
...
static int form_edit_dlg_load_html_file(char *filename, struct form_edit_dlg_t *fedlg)
{
  ...
  for(fedlg->formsnum = 0;; fedlg->formsnum++)
  {
    ...
    formdata = form_get_text(fedlg->formsnum, fedlg->docdata, fedlg->doclen, &formlen);
    ...
    gtk_signal_connect(GTK_OBJECT(mi), "activate", GTK_SIGNAL_FUNC(form_edit_dlg_switch_form), fedlg);
    gtk_widget_show(mi);
    gtk_menu_append(GTK_MENU(menu), mi);
...
static void form_edit_dlg_switch_page(GtkWidget * w, GtkNotebookPage * pg, gint pgnr, struct form_edit_dlg_t *fedlg)
{
  if(pgnr == 1)
  {
    if(fedlg->docdata)
    ...
    if(GTK_CLIST(fedlg->url_list)->selection)
    {
      ...
      urlp = (url *) gtk_clist_get_row_data(GTK_CLIST(fedlg->url_list), GPOINTER_TO_INT(GTK_CLIST(fedlg->url_list)->selection->data));
...
static void form_edit_dlg_load_file_ok(GtkWidget * w, struct form_edit_dlg_t *fedlg)
{
  char *p = (gchar *) gtk_file_selection_get_filename(GTK_FILE_SELECTION(fedlg->fs));
...
static void form_edit_dlg_load_file(GtkWidget * w, struct form_edit_dlg_t *fedlg)
{
  if(!fedlg->fs)
  {
    fedlg->fs = gtk_file_selection_new(gettext("Pavuk: load form file"));

    gtk_signal_connect(GTK_OBJECT(fedlg->fs), "destroy", GTK_SIGNAL_FUNC(gtk_widget_destroyed), &fedlg->fs);
    ...
    gtk_signal_connect(GTK_OBJECT(GTK_FILE_SELECTION(fedlg->fs)->cancel_button), "clicked", GTK_SIGNAL_FUNC(PopdownW), fedlg->fs);
  }
...
static void form_edit_dlg_fill(struct form_edit_dlg_t *fedlg)
{
  ...
  gtk_table_attach(GTK_TABLE(pbox), label, 0, 1, 0, 1, GTK_FILL, GTK_FILL, 2, 2);
  gtk_widget_show(label);

  fedlg->formnr = gtk_option_menu_new();
  gtk_widget_set_usize(fedlg->formnr, 45, -1);
  gtk_table_attach(GTK_TABLE(pbox), fedlg->formnr, 1, 2, 0, 1, GTK_SHRINK, GTK_SHRINK, 2, 2);
  menu = gtk_menu_new();
  gtk_option_menu_set_menu(GTK_OPTION_MENU(fedlg->formnr), menu);
  ...
  gtk_misc_set_alignment(GTK_MISC(label), 0.0, 0.5);
  gtk_table_attach(GTK_TABLE(pbox), label, 0, 1, 1, 2, GTK_FILL, GTK_FILL, 2, 2);
  gtk_widget_show(label);

  fedlg->action = gtk_label_new("");
  gtk_table_attach(GTK_TABLE(pbox), fedlg->action, 1, 2, 1, 2, GTK_FILL, GTK_FILL, 2, 2);
  gtk_widget_show(fedlg->action);

  label = gtk_label_new(gettext("Request method: "));
  gtk_misc_set_alignment(GTK_MISC(label), 0.0, 0.5);
  gtk_table_attach(GTK_TABLE(pbox), label, 0, 1, 2, 3, GTK_FILL, GTK_FILL, 2, 2);
  gtk_widget_show(label);

  fedlg->method = gtk_label_new("");
  gtk_table_attach(GTK_TABLE(pbox), fedlg->method, 1, 2, 2, 3, GTK_FILL, GTK_FILL, 2, 2);
  gtk_widget_show(fedlg->method);

  label = gtk_label_new(gettext("Request encoding: "));
  gtk_misc_set_alignment(GTK_MISC(label), 0.0, 0.5);
  gtk_table_attach(GTK_TABLE(pbox), label, 0, 1, 3, 4, GTK_FILL, GTK_FILL, 2, 2);
  gtk_widget_show(label);

  fedlg->encoding = gtk_label_new("");
  gtk_table_attach(GTK_TABLE(pbox), fedlg->encoding, 1, 2, 3, 4, GTK_FILL, GTK_FILL, 2, 2);
  ...
  gtk_scrolled_window_set_policy(GTK_SCROLLED_WINDOW(fedlg->swin), GTK_POLICY_ALWAYS, GTK_POLICY_ALWAYS);
  ...
  gtk_signal_connect(GTK_OBJECT(button), "clicked", GTK_SIGNAL_FUNC(form_edit_dlg_load_file), fedlg);
...
static struct form_edit_dlg_t fedlg_info = { NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL };
...
void form_edit_dlg(void)
{
  if(!fedlg_info.top)
  {
    ...
    gtk_window_set_title(GTK_WINDOW(fedlg_info.top), gettext("Pavuk: HTML forms editor"));
    gtk_signal_connect(GTK_OBJECT(fedlg_info.top), "destroy", GTK_SIGNAL_FUNC(gtk_widget_destroyed), &fedlg_info.top);
    ...
    gtk_signal_connect(GTK_OBJECT(fedlg_info.tab), "switch_page", GTK_SIGNAL_FUNC(form_edit_dlg_switch_page), &fedlg_info);
    ...
    gtk_signal_connect(GTK_OBJECT(button), "clicked", GTK_SIGNAL_FUNC(form_edit_dlg_close), &fedlg_info);
  }
  form_edit_dlg_refresh_list(NULL, &fedlg_info);
