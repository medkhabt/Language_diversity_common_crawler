...
void save_global_connection_data(global_connection_info * infop, doc * docp)
{
  ...
  infop->actrec_ref = docp->actrec_ref;
#endif
...
void restore_global_connection_data(global_connection_info * infop, doc * docp)
{
...
#endif
#ifdef INCLUDE_CHUNKY_DoS_FEATURES
  docp->actrec_ref = infop->actrec_ref;
#endif
  ...
  if(infop->ftp_con.control)
  {
    ...
    if((docp->doc_url->type == URLT_FTP ||
      ...
      ((pass && infop->ftp_con.passwd &&
        ...
        (!pass && !infop->ftp_con.passwd)) &&
      ((user && infop->ftp_con.user && !strcmp(user, infop->ftp_con.user)) || (!user && !infop->ftp_con.user)))
    {
      docp->ftp_control = infop->ftp_con.control;
    }
    else
    {
      bufio_puts(infop->ftp_con.control, "QUIT\r\n");
...
#endif
  if(cfg.auth_reuse_nonce && infop->http_con.auth_digest && (docp->doc_url->type == URLT_HTTP || docp->doc_url->type == URLT_HTTPS))
  {
    if(!strcmp(infop->http_con.auth_digest->site,
        url_get_site(docp->doc_url)) && infop->http_con.auth_digest->port == url_get_port(docp->doc_url))
    {
  ...
  if(infop->http_con.connection)
  {
    ...
    if(infop->http_con.proto == docp->doc_url->type && infop->http_con.port == port && !strcmp(infop->http_con.host, host))
    {
