...
static htab_entry dns_htab[DNS_HASH_NUM] = { {NULL} };  /* [i_a] init; not every OS/compiler produces zeroed static vars at compile time. */
...
static void dns_serv_loop(bufio * in, bufio * out)
{
  ...
  cfg.read_timeout = .0;
  cfg.write_timeout = .0;
  DEBUG_NET("DNS server loop resets socket timeout to zero!\n");
  ...
  for(;;)
  {
    ...
    {
      if(len)
        xperror("dns server - protocol error");
      else
        xprintf(0, "dns server - client exited\n");
      exit(1);
    }
...
static int dns_cli_gethostbyname(char *host, int *len, char *addr, int *family)
{
  static int serno = 0;
  dns_request_rec_t req;
  dns_response_rec_t resp;
  ...
  req.serial = serno;
  memcpy(req.hostname, host, TL_MIN(strlen(host) + 1, sizeof(req.hostname) - 1));
...
int dns_gethostbyname(char *name, int *alen, char *addr, int *family)
{
  ...
  for(eptr = dns_htab[hpos].e; eptr != NULL; eptr = eptr->next)
  {
    ...
    if(!strcasecmp(eptr->h_name, name))
    {
       ...
       * in a way invisible to compiler while we were
       * waiting on the condition variable.
       */
      if(eptr->h_length == -1)
      {
        /* if h_length is -1, it means that the
         * previous attempt to resolve this name has
         * failed. We don't bother again.
         */
        UNLOCK_DNS;
        return -1;
      }
  ...
  *family = AF_INET;
...
#endif
  {
...
#elif defined(HAVE_GETHOSTBYNAME_R) && !(defined(__OSF__) || defined (__osf__))
    {
      ...
      if(!gethostbyname_r(name, &shost, sbuf, sizeof(sbuf), &host, &sherrno) && host)
...
struct sockaddr *dns_setup_sockaddr(abs_addr * addr, int port, struct sockaddr_storage *buf, int *sasize)
{
