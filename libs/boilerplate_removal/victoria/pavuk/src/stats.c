...
static void DMP_URLLIST(FILE * f, dllist * lst)
{
  url *urlp;
  char *p;
  dllist *parptr;
...
#ifdef GTK_FACE
static void DMP_URLLIST2(GtkWidget * l, dllist * lst, bool_t ajp)
{
  char pom[8192];
  url *urlp;
  char *p;
  ...
  while(ptr)
  {
    ...
    pom[sizeof(pom) - 1] = 0;
    _free(p);
    p = pom;                    /* [i_a] fix compiler warning */
    i = gtk_clist_append(GTK_CLIST(l), &p);
    ...
    for(parptr = urlp->parent_url; ajp && parptr; parptr = parptr->next)
    {
      ...
      _free(p);
      p = pom;                  /* [i_a] fix compiler warning */
      gtk_clist_append(GTK_CLIST(l), &p);
    }
    UNLOCK_URL(urlp);
    ptr = ptr->next;
  }
...
int stats_fill_spage(const char *filename, void *lst)
{
  ...
  case MODE_NOSTORE:
    ...
    if(filename)
    {
      ...
      if(broken)
      {
        fprintf(f, gettext("Downloaded truncated: %d (%3d%%)\n"), brokennr, (int) (((float) brokennr / (float) total) * 100.0));
        DMP_URLLIST(f, broken);
      }
      if(nferr)
      {
        fprintf(f, gettext("Non fatal errors: %d (%3d%%)\n"), nferrnr, (int) (((float) nferrnr / (float) total) * 100.0));
        DMP_URLLIST(f, nferr);
      }
      if(dexist)
      {
        fprintf(f, gettext("Not found documents: %d (%3d%%)\n"), dexistnr, (int) (((float) dexistnr / (float) total) * 100.0));
        DMP_URLLIST(f, dexist);
      }
      if(ferr)
      {
...
/* here will be clist with active URL fields */
    else if(cfg.xi_face)
    {
      ...
      if(broken)
      {
        ...
        gtk_clist_append(GTK_CLIST(l), &pom);
        DMP_URLLIST2(l, broken, TRUE);
      }
      if(nferr)
      {
        sprintf(pom, gettext("Non fatal errors: %d (%3d%%)\n"), nferrnr, (int) (((float) nferrnr / (float) total) * 100.0));
        gtk_clist_append(GTK_CLIST(l), &pom);
        DMP_URLLIST2(l, nferr, TRUE);
      }
      if(dexist)
      {
        sprintf(pom, gettext("Not found documents: %d (%3d%%)\n"), dexistnr, (int) (((float) dexistnr / (float) total) * 100.0));
        gtk_clist_append(GTK_CLIST(l), &pom);
        DMP_URLLIST2(l, dexist, TRUE);
      }
      if(ferr)
      {
        sprintf(pom, gettext("Documents with fatal errors: %d (%3d%%)\n"), ferrnr, (int) (((float) ferrnr / (float) total) * 100.0));
        gtk_clist_append(GTK_CLIST(l), &pom);
        DMP_URLLIST2(l, ferr, TRUE);
      }
      if(uns)
      {
        sprintf(pom, gettext("Documents with unspecific status: %d (%3d%%)\n"), unsnr, (int) (((float) unsnr / (float) total) * 100.0));
        gtk_clist_append(GTK_CLIST(l), &pom);
        DMP_URLLIST2(l, uns, TRUE);
      }
    }
#endif
    break;
...
static void SaveDia(void)
{
  ...
  if(!sw)
  {
    ...
    gtk_signal_connect(GTK_OBJECT(sw), "destroy", GTK_SIGNAL_FUNC(gtk_widget_destroyed), &sw);

    gtk_signal_connect(GTK_OBJECT(GTK_FILE_SELECTION(sw)->ok_button), "clicked", GTK_SIGNAL_FUNC(Save), sw);

    gtk_signal_connect(GTK_OBJECT(GTK_FILE_SELECTION(sw)->cancel_button), "clicked", GTK_SIGNAL_FUNC(PopdownW), sw);
  }
  gtk_widget_show(sw);
...
static void TryDownload(void)
{
  ...
  url *urlp = (url *) gtk_clist_get_row_data(GTK_CLIST(stat_list),
  ...
  case GDK_BUTTON_PRESS:
    ...
    if(bevent->button == 3)
    {
      if(GTK_CLIST(stat_list)->selection)
      {
        ...
        gtk_menu_popup(GTK_MENU(fdata), NULL, NULL, NULL, NULL, 3, bevent->time);
      }
    }
    break;
  default:
...
void stats_show(void)
{
  ...
  if(!sstatw)
  {
    ...
    gtk_signal_connect(GTK_OBJECT(sstatw), "destroy", GTK_SIGNAL_FUNC(gtk_widget_destroyed), &sstatw);
    ...
    gtk_clist_set_column_title(GTK_CLIST(stat_list), 0, gettext("Status page"));
    ...
    gtk_signal_connect(GTK_OBJECT(button), "clicked", GTK_SIGNAL_FUNC(SaveDia), NULL);
    ...
    gtk_widget_show(button);
    gtk_signal_connect(GTK_OBJECT(button), "clicked", GTK_SIGNAL_FUNC(Refresh), NULL);
    ...
    gtk_widget_show(button);
    GTK_WIDGET_SET_FLAGS(button, GTK_CAN_DEFAULT);
    gtk_signal_connect(GTK_OBJECT(button), "clicked", GTK_SIGNAL_FUNC(PopdownW), sstatw);

    menu = gtk_menu_new();
    guitl_menu_attach(menu, sstatw);
    gtk_widget_realize(menu);

    gtk_signal_connect(GTK_OBJECT(stat_list), "button_press_event", GTK_SIGNAL_FUNC(list_events), menu);
    ...
    gtk_widget_show(dmi);
    gtk_signal_connect(GTK_OBJECT(dmi), "activate", GTK_SIGNAL_FUNC(TryDownload), (gpointer) NULL);
  }
  gtk_clist_freeze(GTK_CLIST(stat_list));
  gtk_clist_clear(GTK_CLIST(stat_list));
  if(stats_fill_spage(NULL, stat_list))
