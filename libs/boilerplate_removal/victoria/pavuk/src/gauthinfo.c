...
{
  char *fn = (gchar *) gtk_file_selection_get_filename(GTK_FILE_SELECTION(data));
...
static void Save(GtkWidget * w, gpointer data)
{
  ...
  gtk_signal_connect(GTK_OBJECT(fn), "destroy", GTK_SIGNAL_FUNC(gtk_widget_destroyed), &fn);

  gtk_signal_connect(GTK_OBJECT(GTK_FILE_SELECTION(fn)->ok_button), "clicked", GTK_SIGNAL_FUNC(SaveAI), fn);

  gtk_signal_connect(GTK_OBJECT(GTK_FILE_SELECTION(fn)->cancel_button), "clicked", GTK_SIGNAL_FUNC(PopdownW), fn);
...
static void Load(GtkWidget * w, gpointer data)
{
  ...
  gtk_signal_connect(GTK_OBJECT(GTK_FILE_SELECTION(fn)->ok_button), "clicked", GTK_SIGNAL_FUNC(LoadAI), fn);
...
static void CopyFromList(GtkObject * object, int row, int col, GdkEvent * event, gpointer data)
{
  char *p;
  int i, j;
  ...
  if(GTK_CLIST(object)->selection)
  {
    ...
    gtk_toggle_button_set_state(GTK_TOGGLE_BUTTON(w_type[atoi(p) ? atoi(p) - 1 : 1]), TRUE);
  }
}
...
static void NewRow(int row, GtkWidget * list)
{
  ...
  ad[0] = prottable[(long) gtk_object_get_user_data(GTK_OBJECT(GTK_OPTION_MENU(w_prot)->menu_item))].urlid;
...
#else
  sprintf(pom, "%d", GTK_TOGGLE_BUTTON(w_type[0])->active ? 1 : (GTK_TOGGLE_BUTTON(w_type[1])->active ? 2 : 3));
#endif
  ad[6] = pom;
...
void gauthinfo_run(void)
{
  ...
  if(!topl)
  {
    ...
    gtk_window_set_title(GTK_WINDOW(topl), gettext("Pavuk: Authorization info editor"));

    gtk_signal_connect(GTK_OBJECT(topl), "destroy", GTK_SIGNAL_FUNC(gtk_widget_destroyed), &topl);
    ...
    gtk_signal_connect(GTK_OBJECT(list), "select_row", GTK_SIGNAL_FUNC(CopyFromList), NULL);
    ...
    gtk_table_attach(GTK_TABLE(tbox), label, 0, 1, 0, 1, GTK_FILL, GTK_FILL, 5, 2);
    ...
    gtk_table_attach(GTK_TABLE(tbox), w_prot, 1, 2, 0, 1, GTK_EXPAND, GTK_FILL, 5, 2);
    gtk_widget_show(w_prot);

    label = gtk_label_new(gettext("Host: "));
    gtk_misc_set_alignment(GTK_MISC(label), 0.0, 0.5);
    gtk_table_attach(GTK_TABLE(tbox), label, 2, 3, 0, 1, GTK_FILL, GTK_FILL, 5, 2);
    gtk_widget_show(label);

    w_host = gtk_entry_new();
    gtk_table_attach(GTK_TABLE(tbox), w_host, 3, 4, 0, 1, GTK_EXPAND, GTK_FILL, 5, 2);
    gtk_widget_show(w_host);

    label = gtk_label_new(gettext("User: "));
    gtk_misc_set_alignment(GTK_MISC(label), 0.0, 0.5);
    gtk_table_attach(GTK_TABLE(tbox), label, 0, 1, 1, 2, GTK_FILL, GTK_FILL, 5, 2);
    gtk_widget_show(label);

    w_user = gtk_entry_new();
    gtk_table_attach(GTK_TABLE(tbox), w_user, 1, 2, 1, 2, GTK_EXPAND, GTK_FILL, 5, 2);
    gtk_widget_show(w_user);

    label = gtk_label_new(gettext("Password: "));
    gtk_misc_set_alignment(GTK_MISC(label), 0.0, 0.5);
    gtk_table_attach(GTK_TABLE(tbox), label, 2, 3, 1, 2, GTK_FILL, GTK_FILL, 5, 2);
    gtk_widget_show(label);

    w_pass = gtk_entry_new();
    gtk_table_attach(GTK_TABLE(tbox), w_pass, 3, 4, 1, 2, GTK_EXPAND, GTK_FILL, 5, 2);
    gtk_widget_show(w_pass);

    label = gtk_label_new(gettext("Base directory: "));
    gtk_misc_set_alignment(GTK_MISC(label), 0.0, 0.5);
    gtk_table_attach(GTK_TABLE(tbox), label, 0, 1, 2, 3, GTK_FILL, GTK_FILL, 5, 2);
    gtk_widget_show(label);

    w_base = gtk_entry_new();
    gtk_table_attach(GTK_TABLE(tbox), w_base, 1, 2, 2, 3, GTK_EXPAND, GTK_FILL, 5, 2);
    gtk_widget_show(w_base);

    label = gtk_label_new(gettext("Realm: "));
    gtk_misc_set_alignment(GTK_MISC(label), 0.0, 0.5);
    gtk_table_attach(GTK_TABLE(tbox), label, 2, 3, 2, 3, GTK_FILL, GTK_FILL, 5, 2);
    gtk_widget_show(label);

    w_realm = gtk_entry_new();
    gtk_table_attach(GTK_TABLE(tbox), w_realm, 3, 4, 2, 3, GTK_EXPAND, GTK_FILL, 5, 2);
    gtk_widget_show(w_realm);


    frame = gtk_frame_new("Scheme");
    gtk_table_attach(GTK_TABLE(tbox), frame, 0, 4, 3, 4, GTK_FILL, GTK_FILL, 5, 2);
    gtk_widget_show(frame);
    ...
    gtk_table_attach(GTK_TABLE(tbox), brow, 0, 4, 4, 5, GTK_EXPAND, GTK_FILL, 5, 5);
    ...
    gtk_signal_connect(GTK_OBJECT(button), "clicked", GTK_SIGNAL_FUNC(Append), list);
    ...
    gtk_signal_connect(GTK_OBJECT(button), "clicked", GTK_SIGNAL_FUNC(Modify), list);
    ...
    gtk_signal_connect(GTK_OBJECT(button), "clicked", GTK_SIGNAL_FUNC(guitl_ListClear), list);
    ...
    gtk_signal_connect(GTK_OBJECT(button), "clicked", GTK_SIGNAL_FUNC(guitl_ListDeleteSelected), list);
    ...
    gtk_signal_connect(GTK_OBJECT(button), "clicked", GTK_SIGNAL_FUNC(Apply), list);

    gtk_signal_connect(GTK_OBJECT(button), "clicked", GTK_SIGNAL_FUNC(PopdownW), topl);
    ...
    gtk_signal_connect(GTK_OBJECT(button), "clicked", GTK_SIGNAL_FUNC(Load), NULL);
    ...
    gtk_signal_connect(GTK_OBJECT(button), "clicked", GTK_SIGNAL_FUNC(Save), NULL);
