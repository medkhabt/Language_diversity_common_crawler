...
int authinfo_load(char *fn)
{
  ...
  {
    xperror1("authinfo_load", mk_native(fn));
    UNLOCK_AUTHINFO;
    return -1;
  }
...
int authinfo_save(char *fn)
{
  ...
  LOCK_AUTHINFO;
  fd = tl_open(fn, O_BINARY | O_WRONLY | O_CREAT | O_TRUNC, S_IRUSR | S_IWUSR);
  if(fd < 0)
  {
    xperror1("authinfo_save", mk_native(fn));
    UNLOCK_AUTHINFO;
    return -1;
  }
...
authinfo *authinfo_match_entry(protocol prot, char *host, int port, char *path, char *realm)
{
  int mlen = -1, len;
  authinfo *ret = NULL;
  authinfo *ai = NULL;
  ...
  while(ptr)
  {
    ...
    if(ai->prot == prot && !strcmp(ai->host, host) && port == ai->port)
    {
      ...
      if((ai->base && !strncmp(path, ai->base, strlen(ai->base))) || !ai->base)
      {
        if(ai->base)
          len = strlen(ai->base);
        else
