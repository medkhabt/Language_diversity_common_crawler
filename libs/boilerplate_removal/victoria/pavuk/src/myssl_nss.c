...
static PRStatus PR_CALLBACK myssl_bufio_connect(PRFileDesc * fd, const PRNetAddr * addr, PRIntervalTime timeout)
{
  return PR_SUCCESS;
}

static PRInt32 PR_CALLBACK myssl_bufio_read(PRFileDesc * fd, void *buf, PRInt32 len)
{
  PRInt32 rv;
  bufio *sock;
...
static PRInt32 PR_CALLBACK myssl_bufio_recv(PRFileDesc * fd, void *buf, PRInt32 len, PRIntn flags, PRIntervalTime timeout)
{
  return myssl_bufio_read(fd, buf, len);
}

static PRInt32 PR_CALLBACK myssl_bufio_write(PRFileDesc * fd, void *buf, PRInt32 len)
{
  PRInt32 rv;
  bufio *sock;
...
static PRInt32 PR_CALLBACK myssl_bufio_send(PRFileDesc * fd, void *buf, PRInt32 len, PRIntn flags, PRIntervalTime timeout)
{
  return myssl_bufio_write(fd, buf, len);
}

static PRInt16 PR_CALLBACK myssl_bufio_poll(PRFileDesc * fd, PRInt16 in_flags, PRInt16 * out_flags)
{
  *out_flags = 0;
...
static PRStatus PR_CALLBACK myssl_bufio_getpeername(PRFileDesc * fd, PRNetAddr * addr)
{
  bufio *sock;
  PRUint32 len;
...
static PRStatus PR_CALLBACK myssl_bufio_getsockopt(PRFileDesc * fd, PRSocketOptionData * data)
{
...
static PRFileDesc *myssl_bufio_get_io_layer(void)
{
  ...
  return PR_CreateIOLayerStub(myssl_bufio_methods.file_type, &myssl_bufio_methods);
}
...
static char *my_ssl_passwd_callback(PK11SlotInfo * slot, PRBool retry, void *arg)
{
  char *passwd = NULL;
  if(!retry && arg)
...
static SECStatus my_ssl_verify_callback(void *arg, PRFileDesc * fd, PRBool check_sig, PRBool is_server)
{
  SECStatus rv;
...
} my_ssl_cipher_tab[] =
{
  ...
  {
  SSL_FORTEZZA_DMS_WITH_FORTEZZA_CBC_SHA, "FORTEZZA_DMS_WITH_FORTEZZA_CBC_SHA"},
  {
  ...
  {
  TLS_DHE_DSS_EXPORT1024_WITH_RC4_56_SHA, "DHE_DSS_EXPORT1024_WITH_RC4_56_SHA"},
  {
...
bufio *my_ssl_do_connect(doc * docp, bufio * socket, ssl_connection * parent_ssl_con)
{
  int rv;
  ssl_connection *con;
  PRFileDesc *model;
  ...
  {
    if(http_dummy_proxy_connect(docp, url_get_site(docp->doc_url), url_get_port(docp->doc_url), priv_cfg.ssl_proxy, cfg.ssl_proxy_port))
    {
