...
{
  const char *fn = gtk_file_selection_get_filename(GTK_FILE_SELECTION(gui_cfg.scn_save_shell));

  if(fn && *fn)
  {
    bufio *fd = bufio_open(fn, O_BINARY | O_WRONLY | O_CREAT);
    cfg_dump(&fd);
...
void gui_build_scenario_saver(int popup)
{
  ...
  gui_cfg.scn_save_shell = gtk_file_selection_new(gettext("Pavuk: Scenario saver"));

  gtk_signal_connect(GTK_OBJECT(gui_cfg.scn_save_shell), "destroy", GTK_SIGNAL_FUNC(gtk_widget_destroyed), &gui_cfg.scn_save_shell);

  gtk_signal_connect(GTK_OBJECT(GTK_FILE_SELECTION(gui_cfg.scn_save_shell)->
      ok_button), "clicked", GTK_SIGNAL_FUNC(SaveScn), gui_cfg.scn_save_shell);

  gtk_signal_connect(GTK_OBJECT(GTK_FILE_SELECTION(gui_cfg.scn_save_shell)->
      cancel_button), "clicked", GTK_SIGNAL_FUNC(guitl_PopdownW), gui_cfg.scn_save_shell);
...
{
  const char *fn = gtk_file_selection_get_filename(GTK_FILE_SELECTION(gui_cfg.scn_load_shell));
  ...
  if(fn && *fn)
  {
    ...
    else
    {
      ...
      gtk_check_menu_item_set_state(GTK_CHECK_MENU_ITEM(gui_cfg.me_debug), cfg.debug);
#endif
#ifdef HAVE_MT
      gtk_check_menu_item_set_state(GTK_CHECK_MENU_ITEM(gui_cfg.immessages), cfg.immessages);
#endif
      gtk_check_menu_item_set_state(GTK_CHECK_MENU_ITEM(gui_cfg.me_quiet), cfg.quiet);
...
void gui_build_scenario_loader(int popup)
{
  ...
  gui_cfg.scn_load_shell = gtk_file_selection_new(gettext("Pavuk: Scenario loader"));

  gtk_signal_connect(GTK_OBJECT(gui_cfg.scn_load_shell), "destroy", GTK_SIGNAL_FUNC(gtk_widget_destroyed), &gui_cfg.scn_load_shell);

  gtk_signal_connect(GTK_OBJECT(GTK_FILE_SELECTION(gui_cfg.scn_load_shell)->
      ok_button), "clicked", GTK_SIGNAL_FUNC(LoadScn), gui_cfg.scn_load_shell);

  gtk_signal_connect(GTK_OBJECT(GTK_FILE_SELECTION(gui_cfg.scn_load_shell)->
      cancel_button), "clicked", GTK_SIGNAL_FUNC(guitl_PopdownW), gui_cfg.scn_load_shell);
...
{
  const char *fn = gtk_file_selection_get_filename(GTK_FILE_SELECTION(gui_cfg.scn_add_shell));
...
void gui_build_scenario_adder(int popup)
{
  ...
  gui_cfg.scn_add_shell = gtk_file_selection_new(gettext("Pavuk: Scenario add"));

  gtk_signal_connect(GTK_OBJECT(gui_cfg.scn_add_shell), "destroy", GTK_SIGNAL_FUNC(gtk_widget_destroyed), &gui_cfg.scn_add_shell);
  ...
  gtk_signal_connect(GTK_OBJECT(GTK_FILE_SELECTION(gui_cfg.scn_add_shell)->
      cancel_button), "clicked", GTK_SIGNAL_FUNC(guitl_PopdownW), gui_cfg.scn_add_shell);
