...
int re_pmatch(re_entry * ree, const char *str)
{
#ifdef HAVE_POSIX_REGEX
...
int re_pmatch_sub(re_entry * ree, const char *str, int nr, int *start, int *end)
{
  int rv = FALSE;
  ...
  if(nr < (ree->preg.re_nsub + 1))
  {
    ...
    pmatch = _malloc((ree->preg.re_nsub + 1) * sizeof(*pmatch));
...
#ifdef HAVE_PCRE_REGEX
  {
    ...
    if(pmatch_nr >= nr)
    {
      pmatch = (int *) _malloc((pmatch_nr + 1) * 3 * sizeof(*pmatch));
      rv = pcre_exec(ree->preg, ree->preg_extra, str, strlen(str), 0, 0, pmatch, (pmatch_nr + 1) * 3) >= 0;
...
int re_pmatch_subs(re_entry * ree, const char *str, int *nr, int **subs)
{
  int rv = FALSE;
...
#ifdef HAVE_POSIX_REGEX
  {
    ...
    if(rv)
    {
      *subs = (int *) _malloc((ree->preg.re_nsub + 1) * 2 * sizeof(*subs));
...
#ifdef HAVE_PCRE_REGEX
  {
    ...
    if(rv)
    {
      *subs = (int *) _malloc((pmatch_nr + 1) * 2 * sizeof(*subs));
